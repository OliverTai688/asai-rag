...
export const ASAI_AGENT_PROTOCOL_MANIFESTS: AgentProtocolManifest[] = [
  ...
  },
  {
    identity: {
      ...
      displayName: "AI Meeting capture, summary, memory chat, and writeback boundary",
      ownerSurface: "AI Meeting / visit notes capture",
      module: "MEETING",
      version: "2026-06-21.amm-006a",
      status: "active",
    },
    capabilities: [
      {
    ...
    auth: {
      sessionType: "app-member",
      scopeDerivation: "AMM-006a BFF derives organization/member scope with requireCurrentMember, requires owner-scoped CLIENT_MEETING sessions or owner-scoped Client records, and keeps meeting snapshots/summaries/memory-chat/writeback previews member-private.",
    ...
    privacy: {
      retention: "AMM-006a persists member-private CLIENT_MEETING sessions, text/final transcript turns, Park-style memory candidates, deterministic cited InterviewMeetingSummary rows, and selected writeback InteractionEvent candidates/tasks. Memory chat is deterministic response-only and does not persist chat transcripts.",
    ...
    proof: {
      ...
      sourceAdoption: {
        ...
        notes: [
          "AMM-006a adopts deterministic meeting writeback confirmation cards and API proof, turning summary decisions/action/open-question items into CRM candidate/insight/task events only after advisor selection.",
          ...
          "Meeting capture, deterministic summary, deterministic memory chat, and deterministic writeback boundary are no-provider; API proof verifies AiUsageLog count is unchanged.",
          "Untracked notes UI and note domain prototype files remain outside accepted scope.",
          "Future summary provider, additional global entrypoints, and meeting workspace writeback UI require separate source/proof slices.",
        ],
      },
      knownBlockers: [
        ...
        "Provider-backed meeting chat, pgvector retrieval, same-client cross-member sharing, and meeting workspace writeback UI remain future AMM slices.",
