...
export const ASAI_AGENT_PROTOCOL_MANIFESTS: AgentProtocolManifest[] = [
  ...
  },
  {
    identity: {
      ...
      displayName: "AI Meeting capture, summary, and memory chat",
      ownerSurface: "AI Meeting / visit notes capture",
      module: "MEETING",
      version: "2026-06-21.amm-004a",
      status: "active",
    },
    capabilities: [
      {
    ...
    auth: {
      sessionType: "app-member",
      scopeDerivation: "AMM-004a BFF derives organization/member scope with requireCurrentMember, requires owner-scoped CLIENT_MEETING sessions or owner-scoped Client records, and keeps meeting snapshots/summaries/memory-chat member-private.",
    ...
    privacy: {
      retention: "AMM-004a persists member-private CLIENT_MEETING sessions, text/final transcript turns, Park-style memory candidates, and deterministic cited InterviewMeetingSummary rows. Memory chat is deterministic response-only and does not persist chat transcripts.",
    ...
    proof: {
      ...
      sourceAdoption: {
        ...
        notes: [
          ...
          "Meeting capture, deterministic summary, and deterministic memory chat are no-provider; API proof verifies AiUsageLog count is unchanged.",
          "Untracked notes UI and note domain prototype files remain outside accepted scope.",
          "Future summary provider, additional global entrypoints, and writeback routes require separate source/proof slices.",
        ],
      },
      knownBlockers: [
        ...
        "Provider-backed meeting chat, pgvector retrieval, same-client cross-member sharing, and writeback boundaries remain future AMM slices.",
