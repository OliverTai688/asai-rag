...
export const ASAI_AGENT_PROTOCOL_MANIFESTS: AgentProtocolManifest[] = [
  ...
  },
  {
    ...
    capabilities: [
      ...
      {
        id: "route-b-feedback-family-profile-advisor-context",
        label: "Consume Route B feedback family profile context",
        summary:
          "Turns owner-scoped Route B feedback review family profile grounding into advisor follow-up cards and SPIN evidence without exposing raw theater/session/source ids or writing relationship graph, VisitPlan, client profile, policy, or confirmed CRM facts.",
        humanTrigger: "Advisor prepares another visit after Route B feedback reviewed family/person profile facts, inferences, or unknowns.",
      },
      {
      ...
      },
      {
        ...
        summary:
          "Turns accepted AI Meeting quick notes, summaries, writeback candidates, and Route B feedback-advisor review context into visit-preparation relationship confirmation signals without provider calls, DB persistence, or direct confirmed CRM fact writes.",
        humanTrigger: "Advisor syncs a meeting quick note, reviews a meeting summary, or inspects theater-backed writeback context that should inform the next preparation package.",
      },
      {
    ...
    interfaces: {
      endpoints: [
        ...
        {
          id: "visit-route-b-feedback-advisor-context",
          route: "/api/visits/[id]/route-b-feedback-advisor-context",
          methods: ["GET"],
          providerPosture: "deterministic-no-provider",
          launchPosture: "available",
          modalities: ["structured-json"],
      ...
      actions: [
        ...
        {
          id: "route-b-feedback-family-profile-advisor-context",
          label: "Route B feedback family profile context bridge",
          actionBoundary:
            "Reads owner-scoped TheaterRouteBFeedbackReview.familyProfileGrounding through visitPlanId -> routeBSourcePacketId only; outputs requiresConfirmation=true advisor follow-up cards and VisitQuestionEvidence while keeping providerCallAttempted=false, writesRelationshipGraph=false, writesVisitPlan=false, writesClientProfile=false, writesPolicy=false, and writesConfirmedCrmFact=false.",
        },
        {
          id: "relationship-graph-prep-confirmation-cards",
        ...
        },
        {
          ...
          actionBoundary:
            "Consumes only accepted meeting quick-note/summary/writeback safe DTO text, source refs, and MeetingWritebackCandidateReviewContext summaries, redacts contact/policy/raw-payload sentinels, outputs relationship confirmation signal cards, and keeps writesRelationshipGraph=false, writesVisitPlan=false, persistedToDatabase=false, and writesConfirmedCrmFact=false.",
        },
        {
          id: "meeting-relationship-signal-theater-handoff-grounding",
    ...
    proof: {
      ...
      sourceAdoption: {
        ...
        evidenceRefs: [
          ...
          "route-b-feedback-family-profile-advisor-context",
          "VisitRouteBFeedbackAdvisorContext.outputContract.requiresAdvisorConfirmation=true",
          ...
          "/api/visits/[id]/route-b-feedback-advisor-context",
          "LV3RouteBStateProposalCrossFlow.proofCommand=pnpm lv3:route-b-state-proposal-cross-flow-qa",
          "LV3RouteBStateProposalCrossFlow.surfaces=theater|visit-preparation|ai-meeting-notes|meeting-writeback-preview",
        ...
        notes: [
          ...
          "`pnpm lv3:route-b-state-proposal-cross-flow-qa` now bundles the accepted Route B state proposal path across theater persistence, visit preparation context, AI Meeting notes consumption, and meeting writeback preview. It runs only deterministic source/static proofs and records providerCallAttempted=false, dbConnectionAttempted=false, browserLaunched=false, writesRelationshipGraph=false, writesVisitPlan=false, and writesConfirmedCrmFact=false.",
  ...
  },
  {
    ...
    capabilities: [
      ...
      {
        id: "theater-client-data-build-review",
        label: "Client data theater build review",
        summary:
          "Builds a deterministic client-data theater setup review from owner-scoped client, relationship graph, policy, compliance, and AI tag signals without provider calls or CRM writes.",
        humanTrigger: "Advisor selects a client on /theater to review known facts, inferences, unknowns, and high-sensitivity boundaries before entering theater setup.",
      },
    ],
    interfaces: {
      endpoints: [
        ...
        { id: "theater-client-build-list", route: "/api/theater/client-builds", methods: ["GET"], providerPosture: "deterministic-no-provider", launchPosture: "available", modalities: ["metadata", "structured-json"] },
        { id: "theater-client-build-detail", route: "/api/theater/client-builds/[clientId]", methods: ["GET"], providerPosture: "deterministic-no-provider", launchPosture: "available", modalities: ["metadata", "structured-json"] },
      ],
  ...
  },
  {
    identity: {
      ...
      module: "THEATER",
      version: "2026-06-25.client-route-b-stage-render-harness",
      status: "active",
    },
    capabilities: [
      {
      ...
      {
        id: "route-b-client-next-turn-context",
        label: "Client build next-turn context",
        summary:
          "Transforms client-data Route B handoff review packets into provider-disabled next-turn drafts and provider prompt context with family profile grounding, high-sensitive blocking, and no provider, DB, append-candidate, or confirmed CRM fact writes.",
        humanTrigger: "Advisor previews the next Route B theater move after starting from an owner-scoped client-data theater build.",
      },
      {
        id: "route-b-client-session-source-review",
        label: "Client build session source review",
        summary:
          "Reviews client-data Route B handoff and next-turn source grounding before persisted session adoption, preserving fact/inference/unknown counts and no provider, DB, source-grounding persistence, raw reference, or confirmed CRM fact writes.",
        humanTrigger: "Advisor or reviewer inspects whether a client-built Route B theater session has safe source grounding before persistence is enabled.",
      },
      {
        id: "route-b-client-session-source-consumption",
        label: "Client build session source consumption",
        summary:
          "Shapes client-data Route B session source review payloads into advisor-visible stage source panel contracts with Route B data hooks, fact/inference/unknown counts, high-sensitive blocking, and no provider, DB, source persistence, raw reference, or confirmed CRM fact writes.",
        humanTrigger: "Advisor or reviewer checks whether a client-built Route B session source review can be safely consumed by the Route B stage source panels.",
      },
      {
        id: "route-b-client-stage-source-fixture",
        label: "Client build stage source fixture",
        summary:
          "Adapts client-data Route B source consumption into deterministic stage fixture contracts and scans the Route B session page for matching component/data-attribute hooks without provider calls, DB writes, raw references, or confirmed CRM fact writes.",
        humanTrigger: "Reviewer validates that client-built Route B source panels can be inspected against the actual Route B stage source UI contract while DB proof is unavailable.",
      },
      {
        id: "route-b-client-stage-render-harness",
        label: "Client build stage render harness",
        summary:
          "Renders client-data Route B stage source fixture panels into a deterministic no-DB browser harness and verifies visible source grounding affordances, labels, mobile layout, and no-write privacy boundaries.",
        humanTrigger: "Reviewer verifies that a client-built Route B source panel is not only hooked in source but actually renderable and inspectable in a browser without DB or provider access.",
      },
      {
      ...
      {
        id: "route-b-family-profile-runtime-grounding",
        label: "Family profile runtime grounding",
        summary:
          "Converts persisted relationship-graph family profile source metadata into least-disclosure next-turn and provider context with safe member counts, field counts, status counts, field summaries, unknown prompts, and no-write boundaries only.",
        humanTrigger: "Advisor generates or inspects the next Route B theater turn after creating a stage from relationship graph family profile data.",
      },
      {
      ...
      {
        id: "route-b-feedback-family-profile-grounding",
        label: "Route B feedback family-profile grounding",
        summary:
          "Carries relationship-graph family profile fields into session-end feedback review and feedback provider input as least-disclosure counts, status splits, safe field summaries, unknown prompts, and no-write boundaries only.",
        humanTrigger: "Advisor reviews a Route B practice run and needs feedback to cite known, inferred, and unknown family profile context without writing client profile, policy, relationship graph, VisitPlan, or confirmed CRM facts.",
      },
      {
    ...
    interfaces: {
      ...
      actions: [
        ...
        {
          id: "route-b-client-session-source-review",
          label: "Client build to Route B session source review",
          actionBoundary:
            "Accepts only provider-disabled client Route B next-turn snapshots, reviews scene source grounding for future owner-scoped session persistence, and performs no provider call, DB write, source-grounding persistence, raw source reference disclosure, client/profile/policy write, or confirmed CRM fact write.",
        },
        {
          id: "route-b-client-session-source-consumption",
          label: "Client build Route B source review consumption",
          actionBoundary:
            "Accepts only deterministic ClientRouteBSessionSourceReview payloads, outputs advisor-visible stage panel hooks and counts for /theater/[sessionId], and performs no provider call, DB write, source-grounding persistence, raw source reference disclosure, client/profile/policy write, or confirmed CRM fact write.",
        },
        {
          id: "route-b-client-stage-source-fixture",
          label: "Client build Route B stage source fixture",
          actionBoundary:
            "Accepts only deterministic RouteBSessionSourceReviewConsumption panel contracts, scans the existing Route B session page for matching component/data-attribute hooks, and performs no provider call, DB write, source-grounding persistence, raw source reference disclosure, or confirmed CRM fact write.",
        },
        {
          id: "route-b-client-stage-render-harness",
          label: "Client build Route B stage render harness",
          actionBoundary:
            "Accepts only deterministic ClientRouteBStageSourceAdapterFixture panels, renders safe stage source grounding HTML for browser verification, and performs no provider call, DB write, source-grounding persistence, raw source reference disclosure, raw metadata disclosure, or confirmed CRM fact write.",
        },
        {
          id: "route-b-meeting-signal-source-grounding",
          label: "Meeting signal source grounding",
        ...
        {
          id: "route-b-family-profile-source-grounding",
          label: "Family profile source grounding",
          actionBoundary:
            "Accepts only allowlisted family member profile fields from the owner-scoped relationship graph handoff, persists them as Route B session source metadata, and exposes only person/relation/field/value/status plus source counts without raw metadata, source reference ids, provider calls, relationship graph writes, VisitPlan writes, or confirmed CRM fact writes.",
        },
        {
          id: "route-b-director",
        ...
        },
        {
          ...
          actionBoundary: "Consumes persisted snapshot summaries, meeting signal runtime grounding, relationship edge shadow runtime grounding, family profile runtime grounding, and latest advisor turn into a least-disclosure character/narrator draft; generated text remains disabled until provider logging proof exists.",
        },
        {
          id: "route-b-next-turn-ui-preview",
        ...
        {
          id: "route-b-feedback-family-profile-grounding",
          label: "Feedback family-profile grounding",
          actionBoundary:
            "Consumes RouteBSessionSnapshot.scene.sourceGrounding.familyProfiles inside feedback contract, feedback provider input, and feedback review as safe field-count/status evidence only; no raw metadata, source reference ids, provider call, DB write, client profile write, policy write, relationship graph write, VisitPlan write, or confirmed CRM fact write.",
        },
        {
          id: "route-b-red-line-action-feedback-consumption",
        ...
        },
        {
          ...
          actionBoundary: "Supplies selected objection cues, meeting signal runtime grounding, relationship edge shadow runtime grounding, family profile runtime grounding, and all red-line rules to next-turn/feedback provider inputs without treating roleplay cues as confirmed CRM facts or legal advice.",
        },
        {
          id: "route-b-family-profile-runtime-grounding",
          label: "Family profile runtime grounding",
          actionBoundary: "Consumes persisted family profile source grounding as next-turn/provider prompt evidence only; exposes safe member counts, field counts, status counts, field summaries, unknown prompts, and no-write boundaries without raw metadata, source reference ids, DB writes, provider calls, relationship graph writes, VisitPlan writes, or confirmed CRM fact writes.",
        },
        {
    ...
    schemas: {
      inputDtoRefs: [
        ...
        "BuildClientRouteBSessionSourceReviewInput",
      ...
      outputDtoRefs: [
        ...
        "RouteBStageSourceRenderedPanel",
    ...
    proof: {
      ...
      sourceAdoption: {
        ...
        notes: [
          ...
          "Family-profile feedback QA proves persisted sourceGrounding.familyProfiles is cited in feedback contract, feedback provider prompt context, deterministic review evidence, and stage UI as safe counts/status summaries only, without raw metadata, source reference ids, provider calls, DB writes, client profile writes, policy writes, relationship graph writes, VisitPlan writes, or confirmed CRM fact writes.",
          ...
          "`pnpm lv3:route-b-state-proposal-cross-flow-qa` proves Route B persisted statePatches still feed only visit preparation, AI Meeting notes, and meeting writeback preview context; it does not connect to DB, launch browser, call providers, fake AiUsageLog, write relationship graph, write VisitPlan, or write confirmed CRM facts.",
  ...
  },
  {
    identity: {
      ...
      module: "MEETING",
      version: "2026-06-24.route-b-state-proposal-cross-flow-proof",
      status: "active",
    },
    capabilities: [
      {
      ...
      {
        id: "meeting-route-b-state-proposal-context-consumption",
        label: "Meeting Route B state proposal context consumption",
        summary: "Reads owner-scoped Route B state proposals from the visit BFF, appends safe follow-up/evidence reminders to the meeting note draft, and displays no-provider/no-write guardrails without raw theater IDs.",
        humanTrigger: "Advisor opens post-visit notes for a preparation package with Route B scene or turn state proposals.",
      },
      {
        id: "meeting-route-b-feedback-advisor-context-consumption",
        label: "Meeting Route B feedback family profile consumption",
        summary: "Reads owner-scoped Route B feedback family profile advisor context from the visit BFF, appends safe follow-up reminders to the meeting note draft, and displays no-provider/no-write guardrails without raw theater IDs.",
        humanTrigger: "Advisor opens post-visit notes for a preparation package with Route B feedback review family profile context.",
      },
      {
        id: "meeting-route-b-feedback-advisor-writeback-bridge",
        label: "Route B feedback family profile to meeting writeback preview bridge",
        summary: "Turns safe Route B feedback family profile advisor context into meeting writeback preview context cards, then attaches matching cards as candidate-review context that requires a persisted summary and advisor confirmation before any writeback review.",
        humanTrigger: "Advisor reviews meeting writeback candidates after Route B theater feedback produced family profile advisor context.",
      },
      {
        id: "meeting-route-b-state-proposal-writeback-bridge",
        label: "Route B state proposal to meeting writeback preview bridge",
        summary: "Turns safe Route B state proposal context into meeting writeback preview context cards that require a persisted summary and advisor confirmation before any writeback review.",
        humanTrigger: "Advisor reviews meeting writeback candidates after Route B theater practice produced state proposals.",
      },
      {
    ...
    interfaces: {
      ...
      actions: [
        ...
        {
          id: "consume-route-b-state-proposal-context-in-meeting-notes",
          label: "Consume Route B state proposal context in meeting notes",
          actionBoundary: "Notes UI calls the owner-scoped visit state proposal BFF, passes only status/summary/items/proof into MeetingWorkspace, merges requiresConfirmation=true follow-up reminders into the manual-note draft, and never renders theater session ids, person ids, source packet ids, relationship graph writes, VisitPlan writes, or confirmed CRM facts.",
        },
        {
          id: "consume-route-b-feedback-advisor-context-in-meeting-notes",
          label: "Consume Route B feedback family profile context in meeting notes",
          actionBoundary: "Notes UI calls the owner-scoped visit feedback advisor context BFF, passes only status/summary/items/proof into MeetingWorkspace, merges requiresConfirmation=true family profile follow-up reminders into the manual-note draft, and never renders theater session ids, person ids, source packet ids, relationship graph writes, VisitPlan writes, client profile writes, policy writes, or confirmed CRM facts.",
        },
        {
          id: "route-b-feedback-advisor-writeback-preview-bridge",
          label: "Bridge Route B feedback family profile context into meeting writeback preview",
          actionBoundary: "Client UI derives preview-only writeback context from safe feedback family profile advisor cards, attaches matching cards to meeting writeback candidates as review-only context, requires a persisted meeting summary and advisor confirmation, and never writes relationship graph, VisitPlan, client profile, policy, or confirmed CRM facts.",
        },
        {
          id: "route-b-state-proposal-writeback-preview-bridge",
          label: "Bridge Route B state proposals into meeting writeback preview",
          actionBoundary: "Client UI derives preview-only writeback context from safe state proposal cards, requires a persisted meeting summary and advisor confirmation, and never writes relationship graph, VisitPlan, or confirmed CRM facts.",
        },
        {
    ...
    schemas: {
      ...
      evidenceDtoRefs: [
        ...
        "MeetingRouteBStateProposalWritebackBridge.target=MEETING_WRITEBACK_PREVIEW_CONTEXT",
        "MeetingRouteBStateProposalWritebackBridge.safety.providerCallAttempted=false",
        ...
        "MeetingRouteBFeedbackAdvisorWritebackBridge.target=MEETING_WRITEBACK_PREVIEW_CONTEXT",
        "MeetingRouteBFeedbackAdvisorWritebackBridge.safety.providerCallAttempted=false",
    ...
    proof: {
      ...
      notes: [
        ...
        "AMM/ITA-003p adopts Route B state proposal downstream consumption for AI meeting notes: /pre-visit/[planId]/notes reads `GET /api/visits/[id]/route-b-state-proposal-context`, passes only status/summary/items/proof to MeetingWorkspace, appends requiresConfirmation=true follow-up/evidence reminders to the manual-note draft, and renders no-provider/no-relationship-graph/no-VisitPlan/no-direct-CRM-write guardrails without raw theater session/person/source packet ids.",
        "AMM/ITA-003q adds `MeetingRouteBStateProposalWritebackBridge`, turning accepted Route B state proposal context into meeting writeback preview context only. The bridge requires a persisted meeting summary and advisor confirmation, keeps providerCallAttempted=false, does not require a fake AiUsageLog, and does not write relationship graph, VisitPlan, or confirmed CRM facts. `pnpm meeting:route-b-state-proposal-writeback-bridge-qa` is the source contract proof.",
        "AMM/REL-006h adds `MeetingRouteBFeedbackAdvisorWritebackBridge`, turning Route B feedback family profile advisor context into meeting writeback preview context only. This loop extends the same no-provider source contract with `MeetingWritebackCandidateReviewContext`, so matching bridge cards appear inside meeting writeback candidate review as supporting context without changing POST writeback payloads. The bridge and candidate review context require a persisted meeting summary and advisor confirmation, keep providerCallAttempted=false, do not require a fake AiUsageLog, and do not write relationship graph, VisitPlan, client profile, policy, or confirmed CRM facts. `pnpm meeting:route-b-feedback-advisor-writeback-bridge-qa` is the source contract proof.",
        "`pnpm lv3:route-b-state-proposal-cross-flow-qa` now bundles ITA-003o/p/q into one no-provider/no-DB/no-browser proof across theater persisted state proposals, visit preparation context, AI Meeting notes consumption, and meeting writeback preview. The proof is evidence only and still does not create writeback candidates, write relationship graph, write VisitPlan, or write confirmed CRM facts.",
