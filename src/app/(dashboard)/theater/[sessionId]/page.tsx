...
export default function TheaterSimulationPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/mock/ai/theater", {
        method: "POST",
        body: JSON.stringify({
  ...
  const handleEndSimulation = () => {
    const finalScore = theaterService.generateMockScore();
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6">
        ...
        <div className="md:col-span-4 flex flex-col justify-center space-y-2">
           <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
              ...
              <Activity className={cn("w-3 h-3", session.tension > 70 ? "text-red-500" : "text-indigo-400")} />
           </div>
           <Progress value={session.tension}>
              <ProgressTrack className="h-2 bg-zinc-100 dark:bg-zinc-800 rounded-full">
                <ProgressIndicator className={cn(
                  ...
                  (session.tension > 40 ? "bg-orange-400" : "bg-indigo-500")
                )} />
              </ProgressTrack>
           </Progress>
        </div>
            ...
            <div className="pt-4 space-y-8">
                ...
                <div key={turn.id} className={cn("flex flex-col animate-in fade-in slide-in-from-bottom-2", turn.role === 'agent' ? "items-end" : "items-start")}>
                  <div className={cn(
                    ...
                    <div className={cn(
                      ...
                      turn.role === 'agent' 
                        ? "bg-indigo-50 dark:bg-indigo-900/10 text-indigo-900 dark:text-indigo-100 rounded-tr-sm" 
              ...
              {isTyping && (
                <div className="flex items-center gap-3">
                   <div className="w-10 h-10 rounded-2xl bg-indigo-600 border border-indigo-700 flex items-center justify-center text-white shrink-0 shadow-lg scale-90">
            ...
            {!isCompleted ? (
              <div className="bg-white dark:bg-zinc-900 p-2 rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-2xl ring-1 ring-zinc-100 dark:ring-zinc-800">
                ...
                <div className="flex items-center justify-between px-3 pb-3">
                   <div className="flex items-center gap-2">
                      <Button variant="ghost" size="sm" className="rounded-xl text-[10px] uppercase font-black text-zinc-400" onClick={handleEndSimulation}>
                         終止模擬
                      </Button>
                   </div>
                   <Button 
                    className={cn(
                      "rounded-2xl w-12 h-12 shadow-xl shadow-indigo-500/20",
                      input.trim() ? "bg-indigo-600 hover:bg-indigo-700" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400"
                    )}
                    onClick={handleSend}
              ...
              <Button 
                className="w-full rounded-2xl h-14 bg-indigo-600 hover:bg-indigo-700 text-lg font-bold shadow-xl animate-in zoom-in-95 duration-500"
           ...
           <Card className="rounded-3xl border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900 shadow-sm overflow-hidden sticky top-6">
              <CardContent className="p-6 space-y-6">
                    ...
                    <div className="space-y-3">
                       {personaInfo.traits.map(t => (
                         <div key={t} className="flex items-center gap-3 text-sm font-bold text-zinc-700 dark:text-zinc-300">
                            <Target className="w-4 h-4 text-indigo-500 shrink-0" /> {t}
                         </div>
                       ))}
                    </div>
                 </div>
                 ...
                 <div className="pt-6 border-t border-zinc-100 dark:border-zinc-800">
                    ...
                    <div className="p-4 rounded-2xl bg-white dark:bg-zinc-800 border-2 border-dashed border-indigo-100 dark:border-indigo-900/20 text-xs font-medium text-zinc-500 leading-relaxed italic">
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-300">
           <Card className="w-full max-w-3xl rounded-[40px] border-none shadow-2xl overflow-hidden animate-in zoom-in-95 duration-500">
              <div className="bg-indigo-600 p-8 text-white relative">
                 ...
                 <p className="text-indigo-100 font-bold opacity-80">客戶：{session.clientName} | 難度：{session.difficulty}</p>
              </div>
