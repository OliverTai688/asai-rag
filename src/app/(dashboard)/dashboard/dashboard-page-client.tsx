...
function DashboardAgendaPanel({
  ...
  const recentMeetingHref = agenda[0] ? `/pre-visit/${agenda[0].id}/meeting` : primaryAction.href;
    ...
    <Card>
      <CardHeader className="border-b border-hairline bg-card px-5 py-3.5">
        <div className="flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Link
              data-testid="dashboard-meeting-entrypoint"
              href={recentMeetingHref}
              className="inline-flex h-7 items-center gap-1.5 rounded-full border border-hairline bg-paper-2 px-2.5 text-[11px] font-semibold text-foreground transition-colors hover:border-hairline-2 hover:bg-card focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
            >
              <Mic className="h-3.5 w-3.5" strokeWidth={1.5} aria-hidden="true" />
              最近會議
            </Link>
