...
ITA-003j next-turn contract note（2026-06-22）：已完成 persisted Route B session 的下一回合 no-provider draft contract。`src/domains/theater/route-b-next-turn.ts` 消費 `RouteBSessionSnapshot` + latest advisor group/private turn，重用 `buildTheaterRouteBOrchestrationPlan()` 產出 least-disclosure `TheaterRouteBNextTurnDraft`，包含 speaker / addressee / visibility / guard evidence / state proposal count / provider boundary；`GET /api/theater/route-b/sessions/[sessionId]/next-turn` 沿用 member-scoped session read，不回 direct private dialog 或 provider body。`pnpm theater:route-b-next-turn-dry-run` 通過群聊防連續搶話、私聊點名必答、blocked narrator draft、`providerCallAttempted=false`、`aiUsageLogWritten=false`、`writesConfirmedCrmFact=false` 與 no sensitive sentinel。AgentFacts-style manifest 已新增 next-turn capability / endpoint / DTO refs / proof command。尚未完成：live character text generation provider success/error `AiUsageLog` proof、DB-backed owner API live evidence、自動 append character turn confirmation UI。

ITA-003k next-turn UI consumption note（2026-06-22）：已把 next-turn no-provider draft preview 接進 `/theater/[sessionId]` Route B stage。顧問寫入 group/private turn 後會刷新 preview，也可手動讀取；UI 顯示 selected speaker / addressee / visibility / guard evidence / rationale / provider boundary / privacy proof，並清楚標示 `generatedTextAllowed=false`、`providerCallAttempted=false`、`aiUsageLogWritten=false`、`writesConfirmedCrmFact=false`。角色台詞生成與 append confirmation 仍 disabled，直到 live character provider success/error `AiUsageLog` proof 完成。新增 `pnpm theater:route-b-next-turn-ui-contract-qa` 與 AgentFacts-style next-turn UI preview capability。尚未完成：live character text generation provider proof、DB/browser live owner evidence、真正 append character/narrator turn 的確認流程。

ITA-003l next-turn provider logging contract note（2026-06-22）：已完成 character/narrator next-turn provider success/error usage-log contract，不連外、不寫 DB。新增 `src/domains/theater/route-b-next-turn-provider.ts` 與 `pnpm theater:route-b-next-turn-provider-dry-run`，以 injected provider / usage logger 驗證 provider input 最小揭露、blocked draft 不呼叫 provider/不寫 fake usage/不產生 append candidate、success path 先寫 success usage record 才回 generated append candidate、provider error path 先寫 sanitized error usage record 才回 error。Success append candidate 固定 `requiresAdvisorConfirmation=true`、`writesConfirmedCrmFact=false`、`storesRawProviderPayload=false`、`rawPrivateTranscriptIncluded=false`。AgentFacts-style manifest 已新增 provider log contract capability、DTO/evidence refs 與 proof command。尚未完成：live OpenAI/Anthropic route wiring、owner-scoped persisted character/narrator turn append API/UI、DB/browser live proof、external registry publication。

ITA-003m next-turn append confirmation note（2026-06-22）：已完成 owner-scoped persisted append confirmation source contract。新增 `src/domains/theater/route-b-next-turn-append.ts`、`POST /api/theater/route-b/sessions/[sessionId]/append-candidate` 與 `appendRouteBNextTurnCandidateForMember()`，只接受 provider success candidate + safe `usageLogId` + `confirmedByAdvisor=true` + safe flags；append 本身固定 no-provider、不儲存 raw provider payload / raw private transcript、不寫 confirmed CRM fact。`RouteBSessionSnapshot` 也在 BFF 邊界把 legacy turn role 正規化為 Route B actor kind，讓 persisted next-turn draft 可讀最新 advisor turn。`/theater/[sessionId]` next-turn panel 已接上 append-candidate handler，但沒有 provider candidate/usageLogId 時仍保持 disabled，不用 mock success 假裝 live append。新增 `pnpm theater:route-b-next-turn-append-dry-run` 與 AgentFacts-style append confirmation capability / endpoint / DTO/evidence refs。尚未完成：live OpenAI/Anthropic route wiring 產生真實 provider candidate、DB/browser live append proof、external registry publication。

ITA-003f/S1 relationship stage graph note（2026-06-25）：依 `RES-026` 研究把 `/theater/[sessionId]` 的 Route B 演練舞台從 card-grid 升級為視覺化關係舞台圖。新增 presentational `src/components/theater/route-b-stage-graph.tsx`（`RouteBStageGraph`）：焦點客戶置中、NPC 環繞、SVG 髮絲線連回焦點，點人物即切私聊（沿用既有 `onPrivateFocus` → composer PRIVATE scope），保留 latest speaker/被點名高亮與已知/推論/未知 counts。因 `TheaterRouteBRelation` 只有 `summary`、無 source/target character id，刻意不杜撰人物間 edge，只畫 focus→NPC 連線；mobile 用 stacked fallback（`md:hidden`）避免水平 overflow，桌面用 `aspect-[4/3]` 圖。此 slice 不呼叫 provider、不寫 DB、不寫 confirmed CRM fact，guard badge 維持 `providerCallAttempted=false` / `usageLogWritten=false` / `requiresConfirmation=true` / `writesConfirmedCrmFact=false`。驗收：`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed` 通過。尚未完成：live DB-backed browser 截圖（目前 Supabase host DNS `ENOTFOUND` 阻擋持久化 session 讀取）、director/character/feedback provider runtime（S2–S6，需 operator 啟用 `ENABLE_ROUTE_B_THEATER_PROVIDER` 與 success/error `AiUsageLog`）。

LV3-THEATER-MEETING-SIGNAL-RUNTIME-CONTEXT-001 acceptance note（2026-06-24）：已完成並正式驗收 Route B next-turn / provider prompt 對 persisted meeting-derived relationship signals 的 runtime consumption。`buildTheaterRouteBNextTurnDraft()` 從 `RouteBSessionSnapshot.scene.sourceGrounding.meetingRelationshipSignals` 產出 `meetingRelationshipSignalGrounding` safe summary；`buildRouteBProviderPromptContext()` 與 next-turn provider input 將其作為 roleplay/evidence context；`/theater/[sessionId]` next-turn preview 顯示 `data-route-b-next-turn-meeting-signal-runtime-grounding`。Proof 維持 no raw meeting session id/person id/sourceReferenceIds/transcript/provider payload、provider disabled path 不呼叫 provider且不寫假 `AiUsageLog`、live provider path 仍需 success/error THEATER `AiUsageLog`；AgentFacts manifest 保持 `internal-only`，未做 external registry publication。

Whole-product review note（2026-06-24 after runtime bridge calibration）：第五輪校準確認 REL-004a/b/c/d/e/f/g 與 meeting-signal runtime grounding 已關閉上一輪的 Route B source/runtime gaps；目前最高價值仍被 formal `RelationshipEdge` schema approval 與 relationship confirmation persistence A/B/C 決策卡住。若下一輪仍未收到上述 operator/product answer，優先做 `ITA-003o Route B state proposal -> downstream advisor context bridge`：從 owner-scoped `RouteBSessionSnapshot.scene.statePatches` / persisted turn `statePatches` 產出拜訪準備或 AI meeting notes 的 advisor-context follow-up cards，保留 fact/inference/unknown、`requiresConfirmation=true`、`writesConfirmedCrmFact=false`，不寫 relationship graph、VisitPlan 或 confirmed CRM fact，不呼叫 provider、不偽造 `AiUsageLog`。剩餘純瀏覽器截圖或成本型 live click proof 可交由 operator 自行重跑，不要讓下一輪變成純證據搜集。

ITA-003o state proposal downstream advisor context note（2026-06-24）：已完成 Route B persisted state proposal -> visit preparation advisor-context bridge。`VisitRouteBStateProposalContext` 將 persisted `sceneState.statePatches` 與 turn `statePatches` 轉成 advisor follow-up / evidence-needed / next-question cards，固定 `requiresConfirmation=true`、`writesRelationshipGraph=false`、`writesVisitPlan=false`、`writesConfirmedCrmFact=false`。`GET /api/visits/[id]/route-b-state-proposal-context` 由 owner-scoped VisitPlan 重新推導 `routeBSourcePacketId`，再讀同源 Route B session；`/pre-visit/[planId]` 顯示「劇場狀態回帶」panel，標示來源為 theater state proposal，不要求或顯示 raw theater session/person id。Proof：`pnpm visit:route-b-state-proposal-context-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。尚未完成：formal `RelationshipEdge` table、relationship confirmation persistence、live provider proof、external registry publication。

ITA/AMM-003p AI meeting notes state proposal consumer note（2026-06-24）：已完成 Route B persisted state proposal -> AI meeting notes advisor-context bridge。正式落地面維持已接受的 `/pre-visit/[planId]/notes` + owner-scoped `MeetingWorkspace`，不採用 untracked `/notes` local prototype。Notes page 讀取 `GET /api/visits/[id]/route-b-state-proposal-context`，只將 `status`、`summary`、`routeBStateProposalContext` items 與 proof 傳入 MeetingWorkspace；不傳 `sourcePacketId`、theater session id 或 person id。MeetingWorkspace 新增 `meeting-route-b-state-proposal-context` panel，將 requiresConfirmation=true 的 follow-up / evidence-needed / next-question reminders 併入 manual-note draft，並顯示 no-provider、no relationship graph write、no VisitPlan write、no direct CRM fact write guardrails。Proof：`pnpm meeting:route-b-state-proposal-context-qa`、`pnpm visit:route-b-state-proposal-context-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。尚未完成：formal `RelationshipEdge` table、relationship confirmation persistence、live provider proof、external registry publication。

ITA/AMM-003q meeting writeback preview bridge note（2026-06-24）：已完成 Route B persisted state proposal -> AI Meeting writeback preview context bridge。新增 `src/domains/interview/meeting-route-b-state-proposal-writeback-bridge.ts`，把 `VisitRouteBStateProposalContext` 轉成 `MeetingRouteBStateProposalWritebackBridge` / cards，狀態包含 `SUMMARY_REQUIRED` 與 `READY_FOR_ADVISOR_REVIEW`；`MeetingWorkspace` 在摘要與正式寫回卡片之間顯示 `meeting-route-b-state-proposal-writeback-bridge` panel，明確要求 persisted summary + advisor confirmation，且固定 `providerCallAttempted=false`、`aiUsageLogRequired=false`、`writesRelationshipGraph=false`、`writesVisitPlan=false`、`writesConfirmedCrmFact=false`。Proof：`pnpm meeting:route-b-state-proposal-writeback-bridge-qa`、`pnpm ai:protocol-registry-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。此結果仍只是 writeback preview context，不代表 relationship graph/VisitPlan/confirmed CRM fact 寫入，也不代表 formal `RelationshipEdge` schema、relationship confirmation persistence、live provider proof 或 external registry publication 已完成。

ITA/AMM-003r cross-flow proof note（2026-06-24）：已新增 `pnpm lv3:route-b-state-proposal-cross-flow-qa`，把 ITA-003o/p/q 串成同一個可重跑 proof：Route B persisted `statePatches` -> visit preparation advisor context -> `/pre-visit/[planId]/notes` AI Meeting consumer -> `MEETING_WRITEBACK_PREVIEW_CONTEXT` bridge。此 proof 只跑 deterministic source/static QA 與三個既有 subproof，輸出 `providerCallAttempted=false`、`dbConnectionAttempted=false`、`browserLaunched=false`、`writesRelationshipGraph=false`、`writesVisitPlan=false`、`writesConfirmedCrmFact=false`、`externalRegistryPublicationAttempted=false`；同步更新 `asai.theater.route_b`、`asai.visit.preparation_package`、`asai.meeting.prototype` AgentFacts manifest 與 registry QA。此結果仍不代表 formal `RelationshipEdge` schema、relationship confirmation persistence、live provider proof、external registry publication 或任何 confirmed CRM fact write 已完成。

Whole-product review note（2026-06-22 after Route B append confirmation）：第五輪校準確認 Route B 的「顧問寫入 -> next-turn draft -> provider candidate contract -> advisor-confirmed append」已形成 source-backed chain；下一個最高槓桿缺口轉為 `ITA-004c Route B feedback persistence + session-end UI`。下一輪應把既有 feedback contract/provider logging 接成 owner-scoped persisted review 與 `/theater/[sessionId]` 五視角回顧面板，證明 qualitative-only、red-line not-applicable、requiresAdvisorConfirmation、`writesConfirmedCrmFact=false`、no raw private/provider payload、manager/foreign denied 與 no-provider `AiUsageLog` unchanged；不要把剩餘 DB/browser append 截圖蒐集當成主工作。
...
ITA-005b provider prompt-context note（2026-06-22）：已把異議/紅線 library 接進 next-turn provider 與 feedback provider 的 runtime DTO，不呼叫 provider、不寫 DB。新增 `src/domains/theater/route-b-provider-prompt-context.ts` 與 `pnpm theater:route-b-provider-prompt-context-dry-run`，輸出 `RouteBProviderPromptContext`：12 類異議 summary、依角色/情境選取的 objection cues、18 條 red-line cues、5 immediate severe / 13 post-review ids，以及禁止 legal advice、禁止把 roleplay objection 當 confirmed CRM fact、禁止 raw provider/private payload 的 provider boundary。`TheaterRouteBNextTurnProviderInput.promptContext` 與 `TheaterRouteBFeedbackProviderInput.promptContext` 會消費同一 source；feedback provider 的 `redLineReview.allRules` 現在覆蓋 18 條規則。`pnpm theater:route-b-next-turn-provider-dry-run` / `pnpm theater:route-b-feedback-provider-dry-run` 也新增 promptContext checks，仍保留 success/error `AiUsageLog` contract。尚未完成：live provider route wiring、即時嚴重紅線 UI 提示、完整 compliance ops review 與 external registry publication。

ITA-005c severe red-line warning preview note（2026-06-22）：已把 severe immediate 紅線接成 Route B 劇場的 advisor-visible warning preview，不呼叫 provider、不寫 DB、不產生正式違規結論。新增 `src/domains/theater/route-b-severe-red-line-preview.ts` 與 `pnpm theater:route-b-severe-red-line-preview-dry-run`，從 `RouteBProviderPromptContext` 同源挑出 5 條 severe immediate 紅線（代簽、代墊、保證獲利、吸金、未做 KYC 即推商品），輸出 `RouteBSevereRedLineWarningPreview`：watchlist-only、requires evidence or not-applicable、no auto-block、no legal advice、no confirmed CRM fact、no provider/fake `AiUsageLog`。`/theater/[sessionId]` Route B stage 右欄新增「守門紅線」面板，顯示 trigger signals、evidence policy 與 provider/AiUsageLog boundary。AgentFacts manifest 已新增 `route-b-severe-red-line-warning-preview` capability / action / DTO/evidence refs。尚未完成：live provider route wiring、真實即時偵測、完整 compliance ops action flow 與 external registry publication。
...
- 拜訪後筆記已透過 AMM-005c 接入正式 `CLIENT_MEETING` workspace；legacy `VisitPlan.postVisitNotes` / `postVisitAnalysis` 仍保留為手動輸入與相容來源，不再是孤立的唯一真相。
...
- [x] AMM-003b：JSON mode provider 生成正式 `MeetingSummary`；success/error 寫 `AiUsageLog`；quota 429 不呼叫 provider；可重生覆蓋。
...
完成註記（2026-06-21 AMM-003b）：新增 provider JSON summary path，`POST .../summary` 可用 `mode: "PROVIDER_JSON"` 由 stored `CLIENT_MEETING` turns 產生 cited `MeetingSummary`，quota/provider-disabled guard 先於 provider call，success/error 都寫 `AiUsageLog`，並持久化 `provider/model/usageLogId/generatedBy=provider-json`；仍保留 deterministic fallback。`pnpm meeting:summary-provider-qa` 覆蓋 unauth 401、raw provider-like payload 409/no row、provider-disabled 503/no fake usage、quota 429/no provider、forced provider error 502 + error log/no summary row、success 201 + citations + provider/model/usageLogId、overwrite=false 不重打 provider、overwrite=true 更新 row 並新增 success log、manager 404/no log、DB guardEvidence `providerCallAttempted=true` / `storesRawProviderPayload=false` / `writesConfirmedCrmFact=false`。尚未完成 provider-backed memory-chat、workspace writeback UI 與 dashboard/CRM global entrypoints。

### Batch AMM-004 — 跨會議客戶記憶 + 對答
- [x] AMM-004a：擴充 `retrieveInterviewMemories()` 支援可選 `clientId`（跨 session，保留 visibilityScope）；投影 CRM/policy/family/report/prior summary 成 deterministic memory-chat grounding。
- [x] AMM-004a：`POST .../chat`、`POST /api/ai/clients/[id]/memory-chat`，grounding = 本場 transcript + 客戶跨 session 記憶；答案分 fact/inference/unknown + citations。
- [x] AMM-004b：`mode: "PROVIDER_JSON"` live provider memory-chat；success/error 寫 `AiUsageLog`；quota 429 與 provider-disabled 503 不呼叫 provider、不寫假 log。
- [x] AMM-004a API proof：能引用過去會議與 CRM facts；無權客戶/foreign role denied。
...
完成註記（2026-06-21 AMM-004a）：新增 deterministic/no-provider meeting memory-chat repository 與 `/api/ai/meeting/sessions/[sessionId]/chat`、`/api/ai/clients/[clientId]/memory-chat`；答案以 facts / inferences / unknowns 分桶並附 citations，來源包含目前會議記憶、同客戶跨 session memory、persisted meeting summary、least-disclosure CRM client/family/policy/report metadata。`pnpm meeting:memory-chat-qa` 證明 owner 可問本場/客戶記憶、引用 prior summary 與 CRM facts、manager/foreign denied、raw provider sentinel blocked、不回 email/phone/policy id/raw transcript/provider payload，且 no-provider `AiUsageLog` unchanged。Provider-backed live chat、quota 429/503 與 success/error `AiUsageLog` proof 仍未完成。

完成註記（2026-06-21 AMM-004b）：新增 shared provider memory-chat helper 與 `PROVIDER_JSON` mode，兩條 endpoint 皆先過 owner scope、payload guard、grounding safety、quota guard，再呼叫 OpenAI；provider-disabled / quota / raw-payload / auth / manager denied 路徑不寫假 `AiUsageLog`，provider error/success 都寫 `MEETING` / `OPENAI` `AiUsageLog`。新增 `pnpm meeting:memory-chat-provider-qa` 證明 session/client provider memory-chat 成功、forced provider error 502 + error log、quota 429、provider-disabled 503、least-disclosure citations、無 raw transcript/provider/contact/policy leakage、DB usage log trace linkage。Deterministic fallback 仍保留。
...
- [x] `拜訪後筆記` 升級為會議工作台（手動筆記 + 可選即時轉寫 + 自動摘要 + 對答），`postVisitNotes` 相容並存。
...
完成註記（2026-06-21 AMM-005b）：新增 dashboard「最近會議」入口、CRM client detail「AI 會議工作台」入口與 `/crm/[clientId]/meeting` route；`MeetingWorkspace` 可用 `visitPlanId` 或 `clientId` 啟動同一 accepted AMM BFF。新增 `pnpm meeting:global-entrypoints-qa`，證明 dashboard recent meeting 可直接進 `/pre-visit/[planId]/meeting`、CRM 可用 client-scoped meeting workspace、refresh/new-context summary persistence、DB `CLIENT_MEETING.client_id` 正確且 direct CRM meeting 不需 `visitPlanId`、manager 讀/建 foreign meeting 404、raw provider payload 409/no echo、desktop/mobile no overflow、console error 0、no-provider `AiUsageLog` unchanged。AMM-005c 已於 2026-06-23 補上 `/pre-visit/[planId]/notes` 與 `postVisitNotes` compatibility proof。

Whole-product review note（2026-06-21 after AMM-008）：第五輪校準確認 AMM 已有 source/browser/API/DB/provider proof 串過 CRM → visit → AI meeting workspace → provider summary → writeback → provider memory-chat → realtime guard → org aggregate privacy。當時最高槓桿缺口為 `/pre-visit/[planId]/notes` 與 legacy `postVisitNotes` 尚未接上正式 `CLIENT_MEETING` workspace；此 gap 已由 AMM-005c 於 2026-06-23 關閉，後續不得再把 AMM-005c 當未完成核心缺口。
...
- [x] AMM-006b：meeting workspace 顯示 `/writebacks` preview / confirmation cards；顧問可勾選 confirmed / inference / action / unknown 候選，需 reason/riskAccepted 的項目有明確輸入，提交後顯示 created/blocked/skipped。
- [x] AMM-006b：UI 不把 inference 寫成 CRM fact；confirmed 仍只建立 CRM candidate audit event；action item / unknown 只建立 follow-up task。
- [x] AMM-006b Browser/API proof：desktop/mobile console error 0、無水平 overflow、summary missing 顯示「先生成摘要」、manager/foreign denied、raw provider/private sentinel blocked、no-provider `AiUsageLog` unchanged。
- [x] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
...
完成註記（2026-06-21 AMM-006b）：`src/components/meeting/meeting-workspace.tsx` 現在會在摘要就緒後讀取 `/writebacks` preview，顯示低噪音 confirmation cards，支援勾選候選、reason/riskAccepted、高敏感 blocked result 與 created/blocked/skipped 結果。新增 `pnpm meeting:workspace-writeback-ui-qa` 覆蓋 summary-required UI、desktop/mobile console error 0、無水平 overflow、owner createdEvents、manager 404、raw provider/private sentinel 409/no echo、confirmed approved -> CRM candidate audit、inference CRM fact count = 0、action/unknown -> follow-up task、`writesConfirmedCrmFact=false`、no-provider `AiUsageLog` 153->153 unchanged。AMM-004b provider memory-chat、AMM-005c notes/postVisitNotes compatibility 與 AMM-008 cross-state proof pack 皆已完成；剩餘主要為 AMM-007 pgvector/operator path。
...
- **G4a-G4g 已於 2026-06-23/24 REL-004a/b/c/d/e/f/g 完成**：no-schema shadow contract、BFF summary、準備包 -> 劇場 handoff、劇場建立頁 source review、Route B session source-grounding/readback、next-turn/provider prompt context runtime grounding、feedback review/provider grounding 已固化；正式 REL-004 仍需 migration/rollback approval 才能動 schema。
- **REL-006/006b/006c/006d/006e/006f/006g 已於 2026-06-24 完成**：`FamilyMember.metadata.profile` allowlist 可安全表達人物職位/職業、財務依賴、狀態、決策角色與關係脈絡，已串入 relationship graph source review、準備包 -> 劇場建場、Route B session source grounding、next-turn/provider runtime grounding、feedback review/provider grounding，以及準備包/AI Meeting notes 的 advisor context consumption，且 `/crm/[clientId]/relationships` 具備顧問可操作的人物資料編輯器；仍不改 schema、不寫回 VisitPlan/CRM fact/relationship graph/client profile/policy。
...
### Batch REL-004d — Edge shadow theater build source review（不動 schema）
- [x] `/theater/build?visitPlanId=...` 的準備包來源審查解析 `relationship_edge_shadow_summary=true` knownMaterial，顯示 edge-model readiness、candidate count、source member count、type/status counts 與 warning code。
- [x] Source review 明確標示 formal `RelationshipEdge` schema 尚未核可；此區只作 readiness/source review，不回傳草稿邊內容、不寫回 relationship graph、VisitPlan、CRM fact 或 DB。
- [x] 新增 `pnpm visit:edge-shadow-theater-build-qa` 做 source-level proof，覆蓋 handoff material、theater build parser、readiness panel、no-write boundary 與 server-only payload sentinel。
- [x] 跑 `pnpm visit:theater-handoff-dry-run`、`pnpm visit:edge-shadow-theater-build-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-23 REL-004d）：準備包 -> 劇場建立頁現在會把 RelationshipEdge shadow summary 顯示成可人工審查的 readiness panel，讓顧問在建立劇場前看到關係邊候選與 schema/no-write 邊界；仍不暴露 draft edge internals、不改 schema、不呼叫 provider、不寫 DB。

### Batch REL-004e — Edge shadow -> Route B session source grounding（不動 schema）
- [x] Route B session 建立 / snapshot / readback path 帶入 BFF-safe `relationshipEdgeShadow` source-grounding summary；只含 candidate count、type/status counts、warning code、formal schema/no-write boundary，不含 draft edge internals、source/target node id、metadata、source references、private payload。
- [x] `/theater/[sessionId]` 顯示 compact source-grounding / edge readiness panel，讓顧問進入劇場後仍可看到關係邊候選、warning 與「未核可正式 schema、不寫回關係圖」邊界。
- [x] 不寫回 relationship graph、VisitPlan、CRM confirmed fact 或 DB；不呼叫 provider、不偽造 `AiUsageLog`；高敏感資料仍走既有 reason/riskAccepted gate。
- [x] 新增 `pnpm theater:relationship-edge-shadow-session-source-qa` 或等價 source/API proof，覆蓋 create/read path、UI data hook、server-only/private sentinel、no-provider/no-DB/no-write boundary。
- [x] 跑 `pnpm visit:theater-handoff-dry-run`、新增 session source QA、`pnpm ai:protocol-registry-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-004e）：`RelationshipEdge` shadow readiness 現在會從劇場建立頁帶入 Route B `sourceGrounding.relationshipEdgeShadow`，由 session create/readback 保存並在 `/theater/[sessionId]` 顯示成 counts/warnings/boundaries panel。此 slice 不改 Prisma schema、不呼叫 provider、不新增 DB/relationship graph/VisitPlan/CRM fact write，且不暴露 draft edge internals；formal `RelationshipEdge` table 仍需 migration/rollback approval。

### Batch REL-004f — Edge shadow -> Route B next-turn/runtime grounding（不動 schema）
- [x] `buildTheaterRouteBNextTurnDraft()` 從 `RouteBSessionSnapshot.scene.sourceGrounding.relationshipEdgeShadow` 產出 least-disclosure runtime grounding summary，只含 source member/candidate counts、edge type/status counts、warning code 與 formal-schema/no-write boundary。
- [x] `buildRouteBProviderPromptContext()` 消費上述 `relationshipEdgeShadowGrounding` 作為 roleplay readiness evidence；保持 `registryReadiness=internal-only`，不回 raw draft edges、source reference ids、raw private transcript 或 raw provider payload。
- [x] `/theater/[sessionId]` next-turn preview 顯示 compact `edge shadow in runtime` guard panel，讓顧問知道下一回合推理已看到 edge readiness，但仍不可寫回 relationship graph、VisitPlan 或 confirmed CRM fact。
- [x] 不改 Prisma schema、不呼叫 provider、不偽造 `AiUsageLog`、不做 DB write；formal `RelationshipEdge` table 仍需 migration/rollback approval。
- [x] 跑 `pnpm theater:route-b-next-turn-dry-run`、`pnpm theater:route-b-provider-prompt-context-dry-run`、`pnpm theater:route-b-next-turn-ui-contract-qa`、`pnpm theater:relationship-edge-shadow-session-source-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-004f）：`RelationshipEdge` shadow readiness 現在會從 Route B session `sourceGrounding.relationshipEdgeShadow` 進入 next-turn draft 與 provider prompt context，並在 `/theater/[sessionId]` 下一回合預覽中顯示 safe runtime grounding。此 slice 只使用 counts/warnings/boundaries，不暴露 draft edge internals、不改 schema、不呼叫 provider、不寫 DB、不寫 relationship graph/VisitPlan/CRM confirmed fact；AgentFacts manifest 仍為 `internal-only`。

### Batch REL-004g — Edge shadow -> Route B feedback review/provider grounding（不動 schema）
- [x] `buildTheaterRouteBFeedbackContract()` 從 Route B handoff / session source grounding 產出 `relationshipEdgeShadowGrounding`，只含 source/candidate counts、type/status counts、warning code 與 formal-schema/no-write boundary。
- [x] `buildTheaterRouteBFeedbackProviderInput()` / prompt context 消費 edge shadow grounding 作為 session-end feedback evidence；不得回 raw draft edges、source/target node ids、source references、metadata、raw private transcript 或 raw provider payload。
- [x] `buildTheaterRouteBFeedbackReview()` 寫入 feedback-specific grounding 與 `EDGE_SHADOW` evidence，且 `/theater/[sessionId]` feedback review panel 顯示 `data-route-b-feedback-edge-shadow-grounding` safe source panel。
- [x] 不改 Prisma schema、不呼叫 provider、不偽造 `AiUsageLog`、不做 DB write；不得寫回 relationship graph、VisitPlan 或 confirmed CRM fact；formal `RelationshipEdge` table 仍需 migration/rollback approval。
- [x] 跑 `pnpm theater:route-b-feedback-dry-run`、`pnpm theater:route-b-feedback-provider-dry-run`、`pnpm theater:route-b-feedback-review-qa`、`pnpm theater:relationship-edge-shadow-session-source-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-004g）：`RelationshipEdge` shadow readiness 現在會從 Route B session source grounding 進入 feedback contract、feedback provider prompt context 與 persisted feedback review evidence，並在 `/theater/[sessionId]` 回饋審查區顯示 safe grounding panel。此 slice 只使用 counts/warnings/boundaries，不暴露 draft edge internals、不改 schema、不呼叫 provider、不寫 DB、不寫 relationship graph/VisitPlan/CRM confirmed fact；AgentFacts manifest 仍為 `internal-only`。

### Batch REL-004 — 完整 edge model（動 schema，operator 2026-06-25 核可）
- [x] Prisma 新增 `RelationshipEdge`（`sourceNodeId`/`targetNodeId`/`type`/`factStatus`/`label?`/`derivedFrom?`/`backfillKey?`/`metadata?`），帶可驗證 `organizationId`；新增 enum `RelationshipEdgeType`/`RelationshipEdgeFactStatus`；保留 `FamilyMember` 作 compatibility。
- [x] 支援多親、配偶結合、手足、社會關係；主客戶為節點（`client:<id>:primary`），不再依賴幽靈欄位（離婚/監護 type 留待 enum 擴充）。
- [x] Repository / DTO 邊界：`src/lib/clients/relationship-edge-repository.ts`，UI 不直接 import Prisma；read/write 由 `requireCurrentMember` + `canReadClientDetail`/`canWriteClient` server-scoped。
- [x] Backfill idempotent：沿用 REL-004a `buildRelationshipEdgeShadowBackfill`，以 `backfillKey` upsert；既有 `parentMemberId` → `PARENT_OF`，重跑不重複、不動手動邊。
- [x] 動 schema 跑 `pnpm prisma:validate`、`pnpm prisma:generate`、`pnpm exec prisma db push`（dev target，additive in sync）。Rollback：`relationship_edges` 為 additive 表，可獨立 drop 還原，不影響 `family_members`。
- [~] API/persist proof：`GET/POST /api/clients/[id]/relationship-edges` 與 `pnpm client:relationship-edge-persistence-qa` 已建立；**live-DB idempotent backfill SQL proof 通過**（3→3 無重複、in-place upsert、unique index、node id/type 持久化、test rows 清理）。**live HTTP QA 待 dev server**（Next 16 single-dev-server lock + :3000 被佔用）。
- [x] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

進行中註記（2026-06-25 REL-004）：operator 核可動 schema 後落地 `RelationshipEdge` 持久化「節點＋邊」模型。變更檔案 `prisma/schema.prisma`、`src/lib/clients/relationship-edge-repository.ts`、`src/app/api/clients/[id]/relationship-edges/route.ts`、`scripts/client-relationship-edge-persistence-qa.mjs`、`package.json`。殘留：live HTTP QA 待 dev server；renderer 改吃 persisted edges 屬 REL-009 N4。
...
### Batch REL-006 — Family member profile metadata boundary（不動 schema）
- [x] 定義 `FamilyMember.metadata.profile` allowlist contract，支援職位/職業、年收入或財務依賴、人物狀態/決策角色、關係脈絡、factStatus、source refs；不得保存 raw transcript/provider payload、保單號、email/phone、secret/token。
- [x] BFF update/read path 可安全寫入與讀回 family member profile metadata，scope 由 current member / client ownership 推導，不信任 browser raw org/client scope。
- [x] `relationship-graph.ts` / source review 使用 metadata 取代可驗證欄位的 `UNKNOWN`，但缺值仍保留待確認與 inference/unknown label。
- [x] 不改 Prisma schema、不手改 `src/generated`、不寫 relationship graph edge table、不寫 VisitPlan 或 confirmed CRM fact、不呼叫 provider、不偽造 `AiUsageLog`。
- [x] 新增可重跑 QA（建議 `pnpm client:family-member-profile-metadata-qa`），覆蓋 owner write/read、manager/foreign denial、metadata allowlist/private sentinel、no-schema/no-provider/no-confirmed-fact-write boundary。
- [x] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006）：新增 `src/domains/client/family-member-profile.ts` 作為 `FamilyMember.metadata.profile` allowlist contract；`POST/PATCH /api/clients/[id]/family-members` 透過 current member + client ownership 寫入 `metadata.profile`，`toClientDto()` 只讀回 allowlisted `profile` 而不暴露 raw metadata。`relationship-graph.ts` 會使用 profile 的職位/職業、年收入/財務依賴、人物狀態/決策角色與關係脈絡，保留 FACT/INFERENCE/UNKNOWN 與 source refs；缺值仍留待確認。Proof：`pnpm client:family-member-profile-metadata-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。此 slice 不改 Prisma schema、不手改 `src/generated`、不新增 formal `RelationshipEdge`、不寫 VisitPlan 或 confirmed CRM fact、不呼叫 provider、不偽造 `AiUsageLog`。

### Batch REL-006b — Family profile -> preparation/theater handoff source review（不動 schema）
- [x] `buildVisitTheaterHandoff()` 消費 `FamilyMember.metadata.profile` 的 allowlisted FACT/INFERENCE/UNKNOWN 欄位，新增 `sourceCounts.familyProfileFields` 與 `evidenceSummary.familyProfiles`。
- [x] `knownMaterials` 帶入 `family_profile_field=true` stage grounding，只含 field/label/person/relation/value/status/source refs/rationale 與 no-write flags；不帶 raw metadata、email/phone、policy number、raw provider payload、raw private transcript。
- [x] `/theater/build?visitPlanId=...` 準備包來源審查顯示「人物 profile grounding」panel，讓顧問看到人物欄位的已知/待確認狀態與 stage-only/no-write 邊界。
- [x] Handoff warning/missing 明確標示 profile metadata 只作建場 grounding，UNKNOWN 欄位需現場確認；不寫 relationship graph、VisitPlan、CRM confirmed fact 或 DB。
- [x] 新增 `pnpm visit:family-profile-theater-handoff-qa`，並更新 `pnpm visit:theater-handoff-dry-run` 覆蓋 FACT/INFERENCE/UNKNOWN、source count、no-provider/no-DB/no-write/no-raw boundary。
- [x] 跑 `pnpm visit:theater-handoff-dry-run`、`pnpm visit:family-profile-theater-handoff-qa`、`pnpm visit:edge-shadow-theater-build-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006b）：`FamilyMember.metadata.profile` 會從準備包 -> 劇場 handoff 進入 `knownMaterials` 與建場來源審查，FACT 進 confirmed facts、INFERENCE 進 inferred persona、UNKNOWN 保持待確認。`/theater/build` 顯示 compact 人物 profile grounding panel 與 no-write copy。此 slice 不改 Prisma schema、不手改 `src/generated`、不呼叫 provider、不寫 DB、不寫 relationship graph/VisitPlan/CRM confirmed fact；profile 只作 stage grounding。

### Batch REL-006c — Family profile -> Route B session source grounding/readback（不動 schema）
- [x] `buildTheaterRouteBHandoff()` 支援 BFF-safe `sourceGrounding.familyProfiles` summary，只含人物/關係/欄位/值/status/source count 與 no-raw/no-write boundary；不得帶 raw metadata、source reference ids、email/phone、policy number、provider/private payload。
- [x] `/theater/build` 建立 Route B session 時，將 `family_profile_field=true` stage fields 轉成 `TheaterRouteBFamilyProfileGroundingSummary` 並寫入 handoff source grounding。
- [x] Route B session draft / snapshot / readback 保留 `scene.sourceGrounding.familyProfiles`，`/theater/[sessionId]` 顯示 `data-route-b-family-profile-source-grounding` 來源 panel。
- [x] 不改 Prisma schema、不手改 `src/generated`、不呼叫 provider、不新增 relationship graph/VisitPlan/CRM confirmed fact write、不偽造 `AiUsageLog`。
- [x] 新增 `pnpm theater:family-profile-session-source-qa`，並更新 `pnpm theater:route-b-schema-dry-run` 覆蓋 session sceneState/metadata safe family profile grounding。
- [x] 跑 `pnpm theater:family-profile-session-source-qa`、`pnpm theater:route-b-schema-dry-run`、`pnpm ai:protocol-registry-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006c）：`FamilyMember.metadata.profile` 現在可從準備包建場進入 Route B session `sourceGrounding.familyProfiles`，session draft / metadata / readback 與舞台右側 panel 只呈現 safe profile 欄位摘要與 source count，不暴露 source reference id 或 raw metadata。此 slice 不改 Prisma schema、不手改 `src/generated`、不呼叫 provider、不寫 relationship graph/VisitPlan/CRM confirmed fact、不偽造 `AiUsageLog`；AgentFacts manifest 維持 `internal-only`。

### Batch REL-006d — Family profile advisor editor UI（不動 schema）

- [x] `/crm/[clientId]/relationships` 的關係人列表新增 compact「人物資料」編輯 panel，可維護職位/職業、年收入或財務依賴、人物狀態、決策角色、關係脈絡。
- [x] 每個欄位都要求 FACT/INFERENCE/UNKNOWN 狀態與可選 rationale；payload 只帶 allowlisted profile fields 與 relationship_graph source reference，不暴露 raw metadata/source ids。
- [x] Save/clear 走既有 `clientService.updateFamilyMemberRemote()` + `PATCH /api/clients/[id]/family-members/[memberId]`，server 由 current member/client ownership 推導 scope；UI 不供應 organizationId/ownerId/userId/unitId。
- [x] `familyMemberProfileInputSchema` 接受同一穩定 `schemaVersion`，避免 UI typed payload 與 BFF strict input runtime validation 不一致。
- [x] 不改 Prisma schema、不手改 `src/generated`、不新增 formal `RelationshipEdge`、不寫 VisitPlan 或 confirmed CRM fact、不呼叫 provider、不偽造 `AiUsageLog`。
- [x] 新增 `pnpm client:family-member-profile-ui-qa` source/UI contract proof；以 `CLIENT_FAMILY_PROFILE_UI_QA_BASE_URL=http://localhost:3067 pnpm client:family-member-profile-ui-qa --browser` 做 desktop/mobile browser/API readback proof；並回歸 `client:family-member-profile-metadata-qa`、`visit:family-profile-theater-handoff-qa`、`theater:family-profile-session-source-qa`。
- [x] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006d）：關係圖頁可直接補強人物 profile metadata，讓 advisor 不需 raw ID workflow 就能把人物職位、財務依賴、狀態、決策角色與關係脈絡交給 relationship graph source review、準備包與劇場 grounding。Browser proof 建立可辨識 demo/test client 與 family member，從 UI 儲存 profile 後 API readback 成功；截圖：`docs/06_audits-and-reports/screenshots/modern-ui/relationship-graph/rel-006d-family-profile-editor-desktop.png`、`docs/06_audits-and-reports/screenshots/modern-ui/relationship-graph/rel-006d-family-profile-editor-mobile.png`。此 slice 只寫 `FamilyMember.metadata.profile` allowlist；不改 schema、不呼叫 provider、不寫 relationship graph/VisitPlan/CRM confirmed fact、不暴露 raw metadata/source reference internals。

### Batch REL-006e — Family profile -> Route B next-turn/provider runtime grounding（不動 schema）

- [x] `buildTheaterRouteBNextTurnDraft()` 從 `RouteBSessionSnapshot.scene.sourceGrounding.familyProfiles` 產出 least-disclosure runtime grounding summary，只含 profiled member count、field count、FACT/INFERENCE/UNKNOWN counts、safe field summaries、unknown prompts 與 no-write boundary；不得回 raw metadata、source reference ids、email/phone、policy number、raw private transcript 或 raw provider payload。
- [x] `buildRouteBProviderPromptContext()` / next-turn provider input 消費上述 `familyProfileGrounding` 作為 roleplay/evidence context，新增 prompt rule `useFamilyProfilesAsRuntimeEvidence=true`，且不得把 profile inference/unknown 當 confirmed CRM fact。
- [x] `/theater/[sessionId]` next-turn preview 顯示 compact `data-route-b-next-turn-family-profile-runtime-grounding` panel，讓顧問知道下一回合推理已看到人物 profile，但仍不可寫回 relationship graph、VisitPlan 或 confirmed CRM fact。
...
- [x] AgentFacts manifest / registry QA 新增 family-profile runtime context refs，保持 `registryReadiness=internal-only`，不宣稱 external-ready / external-registered。
- [x] 跑 `pnpm theater:route-b-next-turn-dry-run`、`pnpm theater:route-b-provider-prompt-context-dry-run`、`pnpm theater:route-b-next-turn-ui-contract-qa`、`pnpm theater:family-profile-session-source-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006e）：Route B next-turn/provider runtime 現在會從 persisted `sourceGrounding.familyProfiles` 產生 safe `familyProfileGrounding`，包含人物/欄位/status counts、field summaries、unknown prompts 與 no-write boundary；provider prompt context 明確 `useFamilyProfilesAsRuntimeEvidence=true`，live provider success/error path 仍需先寫 THEATER `AiUsageLog`。`/theater/[sessionId]` next-turn preview 新增 `data-route-b-next-turn-family-profile-runtime-grounding` panel；AgentFacts manifest / registry QA 已加入 refs。新增 `pnpm theater:route-b-family-profile-runtime-qa`，並回歸 next-turn/provider/UI/source/registry/BFF/tsc/lint。此 slice 不改 schema、不寫 relationship graph/VisitPlan/confirmed CRM fact、不呼叫 provider、不偽造 `AiUsageLog`。

### Batch REL-006f — Family profile -> Route B feedback review/provider grounding（不動 schema）

- [x] `buildTheaterRouteBFeedbackContract()` 從 Route B session source grounding 產出 safe `familyProfileGrounding`，只含 profiled member count、field count、status counts、safe field labels/summaries、unknown count 與 no-write boundary。
- [x] `buildTheaterRouteBFeedbackProviderInput()` / feedback prompt context 消費 family profile grounding 作為 session-end feedback evidence；不得回 raw metadata、source reference ids、email/phone、policy number、raw private transcript 或 raw provider payload。
- [x] `buildTheaterRouteBFeedbackReview()` 寫入 feedback-specific family profile grounding evidence，並保留 FACT/INFERENCE/UNKNOWN 分流；不得寫 relationship graph、VisitPlan、client profile、policy 或 confirmed CRM fact。
- [x] `/theater/[sessionId]` feedback review panel 顯示 `data-route-b-feedback-family-profile-grounding` safe panel，並標示 provider/DB/graph/VisitPlan/confirmed CRM fact write 皆為 false。
...
- [x] 跑 `pnpm theater:route-b-feedback-dry-run`、`pnpm theater:route-b-feedback-provider-dry-run`、`pnpm theater:route-b-feedback-review-qa`、`pnpm theater:family-profile-session-source-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006f）：Route B feedback contract/provider/review 現在會從 persisted `sourceGrounding.familyProfiles` 產生 safe `familyProfileGrounding`，包含人物/欄位/status counts、safe field summaries、unknown prompts、FACT/INFERENCE/UNKNOWN evidence 與 no-provider/no-DB/no-write boundary。`/theater/[sessionId]` 五視角回顧新增 `data-route-b-feedback-family-profile-grounding` panel，明確標示 raw metadata/source refs/provider/DB/graph/VisitPlan/client profile/policy/confirmed CRM fact write 皆為 false；新增 `pnpm theater:route-b-family-profile-feedback-qa` 串跑 contract/provider/review/UI/source proof。此 slice 不改 schema、不呼叫 provider、不偽造 `AiUsageLog`、不寫 relationship graph/VisitPlan/client profile/policy/confirmed CRM fact。

### Batch REL-006g — Route B feedback family profile -> visit prep / AI Meeting advisor context（不動 schema）

- [x] `buildVisitRouteBFeedbackAdvisorContextFromFeedbackReview()` 將 Route B feedback review 的 family profile grounding 轉成 visit-safe advisor context，只保留人物/欄位/status/安全摘要/待問問題與 no-write proof，不含 raw metadata、source reference ids、theater session id、person id、email/phone、policy number、raw private transcript 或 raw provider payload。
- [x] `/api/visits/[id]/route-b-feedback-advisor-context` 由 current member + visit plan ownership + server-derived theater handoff source packet 查找 Route B feedback review；browser 不供應 theaterSessionId/personId/org scope。
- [x] `buildVisitReasoningTrace()` 消費 advisor context 作為 P/I/N 問題 evidence；S 階段不前置放大劇場回饋，且 inference/unknown 不升格為 confirmed CRM fact。
- [x] `/pre-visit/[planId]` 與 `/pre-visit/[planId]/notes` / AI Meeting workspace 顯示 feedback advisor context panel，並可生成 notes draft；UI 標示 provider/DB/graph/VisitPlan/client profile/policy/confirmed CRM fact write 皆為 false。
- [x] AgentFacts-style manifest / registry QA 新增 visit + meeting internal-only capability/action/endpoint/proof refs；不得宣稱 external-ready / external-registered。
- [x] 跑 `pnpm visit:route-b-feedback-advisor-context-dry-run`、`pnpm visit:route-b-feedback-advisor-context-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm theater:route-b-family-profile-feedback-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006g）：Route B feedback family profile evidence 現在可由 server-owned BFF 轉成準備包/AI Meeting advisor context，準備包問題清單以 `theater_route_b_feedback_profile` evidence source 連回 P/I/N 推論與待確認事項，AI Meeting notes workspace 可用同一 context 產生顧問筆記草稿。此 slice 不改 schema、不呼叫 provider、不寫 fake `AiUsageLog`、不寫 relationship graph/VisitPlan/client profile/policy/confirmed CRM fact；BFF 不接受 browser-supplied theater session/person scope，且 UI/DTO 不暴露 raw source refs 或 raw private/provider payload。

### Batch REL-006h — Route B feedback advisor context -> AI Meeting writeback preview bridge（不動 schema）

- [x] `MeetingRouteBFeedbackAdvisorWritebackBridge` 將 safe `VisitRouteBFeedbackAdvisorContext` 轉成 `MEETING_WRITEBACK_PREVIEW_CONTEXT` cards；只作寫回候選審查旁證，不直接建立候選、不寫 CRM confirmed fact。
- [x] Bridge 要求 persisted meeting summary 與 advisor confirmation；無摘要時狀態為 `SUMMARY_REQUIRED`，不宣稱 ready。
- [x] Meeting workspace 顯示 `data-route-b-feedback-advisor-writeback-bridge` panel，標示 provider/no fake AiUsageLog/relationship graph/VisitPlan/client profile/policy/confirmed CRM fact write 皆為 false。
- [x] AgentFacts-style manifest / registry QA 新增 internal-only capability/action/DTO/proof refs；不得宣稱 external-ready / external-registered。
- [x] 跑 `pnpm meeting:route-b-feedback-advisor-writeback-bridge-qa`、`pnpm visit:route-b-feedback-advisor-context-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm meeting:route-b-state-proposal-writeback-bridge-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

完成註記（2026-06-24 REL-006h）：Route B feedback family profile advisor context 現在可進一步進入 AI Meeting writeback preview bridge，形成「劇場回饋人物脈絡 -> 準備包/AI Meeting notes -> 會議摘要 -> 寫回候選審查旁證」的 no-schema safe chain。此 slice 不改 schema、不呼叫 provider、不寫 fake `AiUsageLog`、不寫 relationship graph/VisitPlan/client profile/policy/confirmed CRM fact；bridge 不暴露 raw theater session/person/source packet id，且仍要求顧問在會議摘要與 writeback UI 中二次確認。

### Batch REL-007 — 建立/edge 完整性 hardening（N1 + N2，不動 schema）
> 2026-06-24 第二輪深度審查（`RES-024` §2.1）新增。N5/N6 已由 REL-001/002 修復；N1/N2/N3/N4 與 N7/N8/N9 殘留仍開放。
- [ ] **N1**：`createFamilyMemberForClient`（POST）對 `parentMemberId` 加 existence / same-client / cycle 驗證，與 PATCH 共用同一套 helper，違反時回 400（`PARENT_NOT_FOUND` / `PARENT_SELF` / `PARENT_CYCLE`）。
- [ ] **N2**：`relation` 收斂到 `RELATION_GENERATION` 鍵或提供同義詞映射；未知關係明確標 `UNKNOWN` generation，不靜默當「同輩」。
- [ ] 渲染端遇到指向不存在 member 的 `parentMemberId` 時 fallback 到主客戶並標記，不產生指向虛空的 edge。
- [ ] 不改 SPIN 狀態機、不改 Theater enum/scoring、不刪合規欄位。
- [ ] API proof：不存在/跨 client/成環 parent → 400/404；正常 → 201（更新 `pnpm client:relationship-graph-write-qa`）。
- [ ] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

### Batch REL-008 — 跨客戶關係網絡：`linkedClientId`（N3，不動 schema）
- [ ] BFF relationship-graph review / edge 推導讀取 `FamilyMember.linkedClientId`，標示「此關係人同時是 CRM 客戶」並附 owner-scoped 可導覽資訊。
- [ ] `RelationshipMap` 對 linked 節點顯示可點擊導覽到 `/crm/[clientId]`（受 `canReadClientDetail` 限制；無權限只標記不可導覽）。
- [ ] edge 推導可表達跨客戶關係候選，與 REL-004a `RelationshipEdgeDraft` 對齊（no-schema summary，不寫 DB）。
- [ ] 隱私：跨客戶連結不得讓 org manager aggregate / client-facing 介面看到對方客戶 email/phone/明細。
- [ ] API/source proof：linked 標記正確、無權限不導覽、sentinel 掃描無 email/phone 外洩。
- [ ] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。

### Batch REL-009 — BFF 收斂 + 效能/a11y/上限 polish（N4 + N5 回歸 + N7/N8/N9，不動 schema）
- [ ] **N4**：relationships 頁改消費 `/api/clients/[id]/relationship-graph` BFF（含 `edgeShadow`），停止 client-side 重算作為主來源，或明確移除死碼風險。
- [ ] **N5 回歸**：刪除長輩/中間節點後不再產生漂浮孤立節點。
- [ ] **N7**：減少每次 mutation 的全量 refetch（受影響子集或 client-side cache patch）。
- [ ] **N8 residual**：ReactFlow canvas 補基本 a11y，尊重 `prefers-reduced-motion`。
- [ ] **N9**：關係人數量上限/去重策略，與劇場 NPC≤4 規範一致或明確分流。
- [ ] Browser/API proof：BFF-consumed 渲染與 client-side 一致；desktop/mobile console error 0、無水平 overflow。
- [ ] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
...
Whole-product review 註記（2026-06-24 after feedback advisor writeback bridge）：`REL-006/006b/006c/006d/006e/006f/006g/006h` 已把人物 profile 從 CRM/relationship graph 帶到 preparation/theater handoff、Route B session source panel、advisor editor、next-turn/provider runtime、feedback review/provider grounding、準備包/AI Meeting notes advisor context，以及 AI Meeting writeback preview bridge。第五輪校準時發現 `pnpm lv3:route-b-state-proposal-cross-flow-qa` 的 meeting state-proposal subproof 因 proof expectation drift 退紅，已修正為同時確認 red-line、state proposal、feedback advisor 三種 reminder 不覆蓋既有筆記。下一個最高未阻擋 slice 建議為 post-REL-006h cross-flow proof pack refresh：更新/擴充 `pnpm lv3:cross-flow-no-provider-qa` 或相鄰 command，讓 clean-flow proof 覆蓋 client -> relationship graph profile editor -> preparation -> Route B theater -> AI Meeting writeback preview context；若 operator 批准 formal `RelationshipEdge` schema/migration 或回答 relationship confirmation A/B/C，則優先處理該批准/決策。

LV3-CROSS-002 註記（2026-06-24 post-REL-006h proof pack refresh）：`pnpm lv3:cross-flow-no-provider-qa` 已擴充納入 family profile metadata/editor、preparation->theater handoff、Route B session/runtime/feedback grounding、visit/AI Meeting feedback advisor context、AI Meeting writeback preview bridge、state-proposal cross-flow regression、AgentFacts registry 與 AI BFF audit；wrapper 也改用 `/api/public/status` 的 `asai.public_status.v1` 簽名確認真正 ASAI dev server，避免誤接其他 localhost app。完整 live DB-backed proof 目前被 `.env` Supabase host `db.wwocdcicvpmbdmqvskzi.supabase.co` DNS `ENOTFOUND` 阻擋，未宣稱通過；同輪 fallback source/contract proofs 已證明 post-REL-006h no-provider/no-DB/no confirmed CRM fact chain 可重跑。DB host 可解析後，重跑 `DEMO_QA_BASE_URL=http://127.0.0.1:<free-port> pnpm lv3:cross-flow-no-provider-qa` 關閉剩餘 live evidence。

Whole-product review 註記（2026-06-24 after state proposal cross-flow）：`ITA/AMM-003r` 已把 Route B state proposal 串到 visit prep、AI Meeting notes 與 writeback preview；REL-006/006b/006c/006d 已補上 relationship graph person metadata 的 BFF、source review、handoff/session grounding 與 advisor UI editor。下一輪若 cadence 進入第五輪校準，優先跑 whole-product gap review；若回到 implementation，建議評估 family profile 是否該進一步進入 Route B next-turn/provider prompt context，或等待 formal `RelationshipEdge` schema/migration approval。

Whole-product review 註記（2026-06-24 fifth-loop calibration）：REL-004a/b/c/d/e/f/g 已把 edge shadow 從 contract、BFF summary、準備包 handoff、劇場建立頁 source review、Route B session readback、next-turn/provider runtime context 推到 feedback review/provider grounding。下一個最高槓桿若 operator 批准 additive schema/migration，回到正式 `REL-004` edge table；若尚未批准，優先處理 relationship confirmation persistence 的 A/B/C 決策或選擇另一條不動 schema 的 LV3 source bridge。

Whole-product review 註記（2026-06-23 fifth-loop calibration）：近四輪已補 Route B meeting-signal source grounding、relationship confirmation persistence blocker analysis 與 payment transaction upsert boundary；核心 client -> relationship -> preparation -> theater 流程的最高未解 source gap 回到正式 edge model。由於 REL-004 需要 migration/rollback approval，本輪把下一張安全卡收斂為 REL-004a：先用 source-backed DTO + executable dry-run proof 固化 edge table 前的 contract，讓後續 schema migration 不再只是 proof-plan。
