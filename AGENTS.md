...
- **G4a-G4f 已於 2026-06-23/24 REL-004a/b/c/d/e/f 完成**：no-schema shadow contract、BFF summary、準備包 -> 劇場 handoff、劇場建立頁 source review、Route B session source-grounding/readback、next-turn/provider prompt context runtime grounding 已固化；正式 REL-004 仍需 migration/rollback approval 才能動 schema。
...
Whole-product review 註記（2026-06-24 fifth-loop calibration）：REL-004a/b/c/d 已把 edge shadow 從 contract、BFF summary、準備包 handoff 推到劇場建立頁 source review；目前最高槓桿缺口改為 session 內 readback。下一輪建議做 `REL-004e`：不動 schema、不呼叫 provider、不寫 DB，把同一份 least-disclosure edgeShadow summary 帶進 Route B session `sourceGrounding`/readback 與 `/theater/[sessionId]`，讓顧問在劇場中仍可看見候選關係邊、warning 與 no-write/formal-schema-not-approved 邊界。
