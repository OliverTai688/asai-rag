...
- **REL-006/006b 已於 2026-06-24 完成**：`FamilyMember.metadata.profile` allowlist 可安全表達人物職位/職業、財務依賴、狀態、決策角色與關係脈絡，並已串入 relationship graph source review 與準備包 -> 劇場建場來源審查；仍不改 schema、不寫回 VisitPlan/CRM fact/relationship graph。
...
Whole-product review 註記（2026-06-24 after state proposal cross-flow）：`ITA/AMM-003r` 已把 Route B state proposal 串到 visit prep、AI Meeting notes 與 writeback preview；下一個未阻擋的最高槓桿缺口回到 relationship graph 的 person metadata。Family member 目前有 `metadata Json` 但 UI/BFF/graph source review 仍將職位、年收入、人物狀態顯示為 `UNKNOWN`。若未收到 REL-004 schema approval 或 relationship confirmation A/B/C answer，下一輪建議做 `REL-006 family/person metadata profile boundary`：不動 schema、不呼叫 provider，先用 allowlisted metadata 讓人物資訊可被準備包與劇場安全消費。
