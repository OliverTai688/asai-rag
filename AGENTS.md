...
進行註記：2026-06-20 RAS-003a 已完成 workspace bootstrap navigation contract。`/api/workspace/bootstrap?surface=...` 現在回傳 `navigation.sidebarSections`、`surfaceSwitches`、`defaultSurfaceRedirect`、`routeGuardAlignment`、`settingsRoutePolicy` 與 no-provider/no-db-write proof；`pnpm nav:workspace-bootstrap-qa` 覆蓋 member、owner、scoped manager、unscoped manager、AGENT 相容角色、quota exhausted disabled state 與 org surface 不混 member routes。尚未完成正式 URL guard/browser auth proof；`requireOrgAdminRoute` 對 manager scope 的 route 行為仍需下一輪 RAS-003b 收斂。
