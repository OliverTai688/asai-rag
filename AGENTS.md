...
- Billing/ECPay production-grade BFF、public status/lead BFF、CRM related-list/archive/update、admin/pilot seed 與 Theater AI hardening 仍待完整化。
...
Research-to-executable note（2026-06-21 after BFF-304a）：BFF-304a 已關閉 platform session / metadata / audit frontier；下一個 public-facing 缺口是 public pages 的狀態與 CTA 邏輯仍未形成單一 public-safe BFF truth。`BFF-305a` 應先補 `/api/public/status` 或等價 contract，讓 maintenance、AI availability、checkout availability、CTA posture、lead capture posture、legal/privacy status 由 server 回傳；`/api/public/pricing` 必須與 status/CTA 一致，landing/pricing 不得只靠 hardcoded frontend copy 判斷是否可試用/結帳。此 slice 不啟用真實付款/email/notification，不新增 production lead 寫入；若 lead endpoint 未具 rate limit/spam/consent proof，必須明確 deferred。
...
### Batch BFF-305a — Public status and CTA availability proof（下一輪建議）
- [ ] 建立 `GET /api/public/status` 或同等 BFF，匿名可讀，回 `maintenance`、`aiAvailability`、`checkoutAvailability`、`primaryCta`、`leadCapture`、`legalStatus`、`updatedAt`。
- [ ] Status DTO 不含 private plan cost、provider raw config、billing internal state、payment transaction、tenant/client data、secret/token/raw provider payload。
- [ ] `/api/public/pricing` 與 public status 共用 checkout / CTA availability source；public pricing 不再和 status 回傳矛盾。
- [ ] Landing / pricing CTA 依 public BFF contract 顯示：private beta / invite / checkout disabled / contact sales / unavailable states。
- [ ] Public lead endpoint 若未具 rate limit、spam protection、consent version 與 safe persistence proof，需明確 deferred；不得新增裸寫入 endpoint。
- [ ] Public status endpoint 不得混同 NANDA / third-party public discovery 或 external registry；agent publication、credential signing、cross-org agent access 仍回 NAP gate 與 operator approval。
- [ ] API proof 覆蓋 status 200、pricing 200、CTA consistency、checkout disabled/sandbox posture、private sentinel 0。
- [ ] Browser proof 覆蓋 landing/pricing desktop/mobile CTA 與 no overflow；不得宣稱 payment/email/notification production ready。
- [ ] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`、targeted public BFF QA（新增或更新 `pnpm public:pricing-qa` / `pnpm public:status-qa`）。
