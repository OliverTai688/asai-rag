...
- **REL-006/006b/006c/006d/006e 已於 2026-06-24 完成**：`FamilyMember.metadata.profile` allowlist 可安全表達人物職位/職業、財務依賴、狀態、決策角色與關係脈絡，已串入 relationship graph source review、準備包 -> 劇場建場、Route B session source grounding、next-turn/provider runtime grounding，且 `/crm/[clientId]/relationships` 具備顧問可操作的人物資料編輯器；仍不改 schema、不寫回 VisitPlan/CRM fact/relationship graph。下一個 no-schema source gap 是 REL-006f：把 `sourceGrounding.familyProfiles` 帶進 Route B feedback review/provider grounding。
...
- [ ] `buildTheaterRouteBFeedbackContract()` 從 Route B session source grounding 產出 safe `familyProfileGrounding`，只含 profiled member count、field count、status counts、safe field labels/summaries、unknown count 與 no-write boundary。
- [ ] `buildTheaterRouteBFeedbackProviderInput()` / feedback prompt context 消費 family profile grounding 作為 session-end feedback evidence；不得回 raw metadata、source reference ids、email/phone、policy number、raw private transcript 或 raw provider payload。
- [ ] `buildTheaterRouteBFeedbackReview()` 寫入 feedback-specific family profile grounding evidence，並保留 FACT/INFERENCE/UNKNOWN 分流；不得寫 relationship graph、VisitPlan、client profile、policy 或 confirmed CRM fact。
- [ ] `/theater/[sessionId]` feedback review panel 顯示 `data-route-b-feedback-family-profile-grounding` safe panel，並標示 provider/DB/graph/VisitPlan/confirmed CRM fact write 皆為 false。
- [ ] Provider-disabled feedback path 不得呼叫 provider、不得寫 fake `AiUsageLog`；live provider path 若使用該 context，success/error 仍必須先寫 THEATER `AiUsageLog` 並只回 safe feedback candidate。
- [ ] 跑 `pnpm theater:route-b-feedback-dry-run`、`pnpm theater:route-b-feedback-provider-dry-run`、`pnpm theater:route-b-feedback-review-qa`、`pnpm theater:family-profile-session-source-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
