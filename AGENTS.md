...
- **G4a-G4d 已於 2026-06-23 REL-004a/b/c/d 完成**：no-schema shadow contract、BFF summary、準備包 -> 劇場 handoff 與劇場建立頁 source review 已固化；正式 REL-004 仍需 migration/rollback approval 才能動 schema。
- **G4e session source-grounding gap**：RelationshipEdge shadow summary 已到劇場建立前的來源審查，但尚未進入 Route B session 的 `sourceGrounding`/readback；顧問進入劇場後無法在同一工作面看到 edge-model readiness、warning 與 no-write 邊界。
