...
Context: 解除 `/theater` 對 legacy SPIN / AI 顧問陪談摘要的唯一入口依賴，讓 AI 劇場可直接用劇場半結構訪綱 B 建場，也可帶客戶資料建場或從既有訪談轉入。研究依據：`docs/07_research-and-design/RES-019_theater-direct-field-guide-gap-framework-v1.0.md`；逐張任務卡：`docs/05_execution-plans/PLN-020_theater-direct-field-guide-batch-tasks-v1.0.md`；驗收：`docs/08_acceptance-and-qa/ACC-012_theater-direct-field-guide-acceptance-framework-v1.0.md`。本條是 ITA-003 Route B 前置切片：先做入口 IA、訪綱 B TS contract、setup draft 與 handoff packet；除 TDF-005 外不改 Theater legacy enum/scoring/schema。

### Current TDF Gaps
- `RES-019` 已確認 AI 劇場的前置條件應是「有可建場素材」，不是「必須先有 AI 顧問陪談 / SPIN」。
- 目前 `/theater` 仍以 `completedSpinSessions` 作唯一來源；無 SPIN 時提示先完成 AI 顧問陪談，產品心智錯誤。
- `RES-003` 劇場場域建構半結構訪綱 B 已存在，但尚未萃取成 `InterviewOutline` TS 常數，也沒有 `TheaterSetupDraft` / material provenance contract。
- Route B 多角色、導演編排、群聊/私聊與五視角回饋仍由 ITA-003/004/006 管理；TDF 先補可 review 的前置切片。
...
- [ ] `/theater` header 文案移除「必須先有 SPIN 摘要」語意，改成「選建場方式 → 確認素材 → 開始演練」。
- [ ] 第一屏新增三入口 selector：`用劇場訪綱建場`、`帶客戶資料建場`、`從既有訪談轉入`。
- [ ] 無 SPIN / interview material 時仍能選 `用劇場訪綱建場`，主 CTA 不因缺 SPIN disabled。
