...
- **REL-006/006b/006c/006d 已於 2026-06-24 完成**：`FamilyMember.metadata.profile` allowlist 可安全表達人物職位/職業、財務依賴、狀態、決策角色與關係脈絡，已串入 relationship graph source review、準備包 -> 劇場建場、Route B session source grounding，且 `/crm/[clientId]/relationships` 具備顧問可操作的人物資料編輯器；仍不改 schema、不寫回 VisitPlan/CRM fact/relationship graph。第五輪校準後下一個 no-schema source gap 是 REL-006e/f：把 `sourceGrounding.familyProfiles` 帶進 Route B next-turn/provider runtime 與 feedback review/provider grounding。
...
- [ ] `buildTheaterRouteBNextTurnDraft()` 從 `RouteBSessionSnapshot.scene.sourceGrounding.familyProfiles` 產出 least-disclosure runtime grounding summary，只含 profiled member count、field count、FACT/INFERENCE/UNKNOWN counts、safe field summaries、unknown prompts 與 no-write boundary；不得回 raw metadata、source reference ids、email/phone、policy number、raw private transcript 或 raw provider payload。
- [ ] `buildRouteBProviderPromptContext()` / next-turn provider input 消費上述 `familyProfileGrounding` 作為 roleplay/evidence context，新增 prompt rule `useFamilyProfilesAsRuntimeEvidence=true`，且不得把 profile inference/unknown 當 confirmed CRM fact。
- [ ] `/theater/[sessionId]` next-turn preview 顯示 compact `data-route-b-next-turn-family-profile-runtime-grounding` panel，讓顧問知道下一回合推理已看到人物 profile，但仍不可寫回 relationship graph、VisitPlan 或 confirmed CRM fact。
...
- [ ] AgentFacts manifest / registry QA 新增 family-profile runtime context refs，保持 `registryReadiness=internal-only`，不宣稱 external-ready / external-registered。
- [ ] 跑 `pnpm theater:route-b-next-turn-dry-run`、`pnpm theater:route-b-provider-prompt-context-dry-run`、`pnpm theater:route-b-next-turn-ui-contract-qa`、`pnpm theater:family-profile-session-source-qa`、`pnpm ai:protocol-registry-qa`、`pnpm ai:bff-audit`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
...
Whole-product review 註記（2026-06-24 family profile runtime calibration）：`REL-006/006b/006c/006d` 已把人物 profile 從 CRM/relationship graph 帶到 preparation/theater handoff、Route B session source panel 與 advisor editor；source audit 顯示 meeting-signal 與 relationship-edge-shadow 已進入 next-turn/provider/feedback runtime，但 `familyProfiles` 尚未進入 `route-b-next-turn.ts`、`route-b-provider-prompt-context.ts` 或 feedback review/provider grounding。下一個最高槓桿安全 slice 是 `REL-006e`，其次是 `REL-006f`；若 operator 批准 formal `RelationshipEdge` schema/migration 或回答 relationship confirmation A/B/C，則優先處理該批准/決策。
