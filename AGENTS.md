...
- **REL-006/006b/006c/006d/006e/006f 已於 2026-06-24 完成**：`FamilyMember.metadata.profile` allowlist 可安全表達人物職位/職業、財務依賴、狀態、決策角色與關係脈絡，已串入 relationship graph source review、準備包 -> 劇場建場、Route B session source grounding、next-turn/provider runtime grounding、feedback review/provider grounding，且 `/crm/[clientId]/relationships` 具備顧問可操作的人物資料編輯器；仍不改 schema、不寫回 VisitPlan/CRM fact/relationship graph/client profile/policy。
...
Whole-product review 註記（2026-06-24 family profile runtime calibration）：`REL-006/006b/006c/006d/006e` 已把人物 profile 從 CRM/relationship graph 帶到 preparation/theater handoff、Route B session source panel、advisor editor 與 next-turn/provider runtime；source audit 顯示 `familyProfiles` 尚未進入 feedback review/provider grounding。下一個最高槓桿安全 slice 是 `REL-006f`；若 operator 批准 formal `RelationshipEdge` schema/migration 或回答 relationship confirmation A/B/C，則優先處理該批准/決策。
