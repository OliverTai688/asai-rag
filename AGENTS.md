...
- **G1 幽靈欄位**：`Client.parentMemberId`（`src/domains/client/types.ts:99`）在型別/UI 被讀寫，但不在 Prisma `Client` model、不在 `toClientDto()`；新增父節點靜默丟失。
...
- [ ] 移除 / 廢用 `Client.parentMemberId`（`src/domains/client/types.ts:99`）與其讀取點：`RelationshipMap.tsx:149-159` client-parent edge、`:180` `isClientParent` dead code、`relationship-graph.ts` `getParentLabel` 依賴。
