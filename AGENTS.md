...
- [ ] Route B session 建立 / snapshot / readback path 帶入 BFF-safe `relationshipEdgeShadow` source-grounding summary；只含 candidate count、type/status counts、warning code、formal schema/no-write boundary，不含 draft edge internals、source/target node id、metadata、source references、private payload。
- [ ] `/theater/[sessionId]` 顯示 compact source-grounding / edge readiness panel，讓顧問進入劇場後仍可看到關係邊候選、warning 與「未核可正式 schema、不寫回關係圖」邊界。
- [ ] 不寫回 relationship graph、VisitPlan、CRM confirmed fact 或 DB；不呼叫 provider、不偽造 `AiUsageLog`；高敏感資料仍走既有 reason/riskAccepted gate。
- [ ] 新增 `pnpm theater:relationship-edge-shadow-session-source-qa` 或等價 source/API proof，覆蓋 create/read path、UI data hook、server-only/private sentinel、no-provider/no-DB/no-write boundary。
- [ ] 跑 `pnpm visit:theater-handoff-dry-run`、新增 session source QA、`pnpm ai:protocol-registry-qa`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
