...
- **G4a whole-product review bridge**：2026-06-23 校準確認正式 edge table 仍是最高槓桿 gap，但 REL-004 動 schema 尚需 migration/rollback approval；下一輪先做 `REL-004a` no-schema shadow contract + backfill dry-run，將 edge DTO、allowlist、idempotent backfill 與 QA command 固化，避免在未核可 DB migration 前空轉。
...
- [ ] 在 domain/repository 邊界定義 server-only `RelationshipEdgeDraft` / backfill DTO（sourceNodeId、targetNodeId、type、factStatus、label?、metadata allowlist、sourceReferenceIds），不改 Prisma schema、不手改 `src/generated`、不寫 DB。
- [ ] Backfill dry-run 從既有 `FamilyMember.parentMemberId` + `relation` 推導未來 `RelationshipEdge` candidates，輸出 deterministic count / duplicate guard / unsupported relationship warnings；重跑結果 idempotent。
- [ ] Metadata allowlist 不含 email、phone、raw private transcript、raw provider payload、policy number、secret/token；edge 只標 FACT/INFERENCE/UNKNOWN 與安全摘要。
- [ ] 新增 `pnpm client:relationship-edge-shadow-qa`，覆蓋單親、雙親、配偶、手足、社會關係、unknown/inference sentinel、no schema/no DB write/no provider proof。
- [ ] 更新 `ACC-016` 的 REL-004a 驗收，並把 REL-004 migration approval / rollback note 保持為正式 schema 前置條件。
- [ ] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
