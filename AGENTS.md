...
- [ ] 定義 `FamilyMember.metadata.profile` allowlist contract，支援職位/職業、年收入或財務依賴、人物狀態/決策角色、關係脈絡、factStatus、source refs；不得保存 raw transcript/provider payload、保單號、email/phone、secret/token。
- [ ] BFF update/read path 可安全寫入與讀回 family member profile metadata，scope 由 current member / client ownership 推導，不信任 browser raw org/client scope。
- [ ] `relationship-graph.ts` / source review 使用 metadata 取代可驗證欄位的 `UNKNOWN`，但缺值仍保留待確認與 inference/unknown label。
- [ ] 不改 Prisma schema、不手改 `src/generated`、不寫 relationship graph edge table、不寫 VisitPlan 或 confirmed CRM fact、不呼叫 provider、不偽造 `AiUsageLog`。
- [ ] 新增可重跑 QA（建議 `pnpm client:family-member-profile-metadata-qa`），覆蓋 owner write/read、manager/foreign denial、metadata allowlist/private sentinel、no-schema/no-provider/no-confirmed-fact-write boundary。
- [ ] 跑 `pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`。
