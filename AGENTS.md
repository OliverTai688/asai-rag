...
- `RES-008` 已完成 AI-first sidebar IA；RAS-004b 已讓 dashboard shell 改吃 `RoleAwareSidebar` + RAS-003 `buildWorkspaceSidebarRenderModel()`，legacy `src/components/layout/sidebar.tsx` 保留但不再是 dashboard shell 主 renderer。
...
- `ARC-006` 已定義 app/client/platform session 分離與 manager aggregate visibility；目前仍缺 RAS-005 live cross-role Browser/session matrix，不能把 fixture proof 宣稱為 production auth 通過。
