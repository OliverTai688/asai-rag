...
| TDF-006 | Cross-state QA and docs sync | [x] | TDF-001..TDF-004 |
...
完成註記（2026-06-20）：新增 `scripts/theater-direct-field-qa.mjs` 與 `pnpm theater:direct-field-qa`，以 headless browser 證明 `/theater` 在無素材狀態可直接點「開始建場」進 `/theater/build`，build page 顯示「場域建構包」、獨立建場與文字/語音建場入口，且不被 source-sensitive gate 阻擋；同一腳本也用 quickstart fixture 證明 legacy SPIN → theater view 轉入仍可用，並串跑 `pnpm theater:route-b-handoff-dry-run`、`pnpm visit:theater-gate-qa`、`pnpm theater:client-build-qa`。截圖：`tdf-006-theater-direct-desktop.png`、`tdf-006-theater-direct-mobile.png`、`2026-06-20-theater-gate-approved-desktop.png`、`2026-06-20-theater-gate-approved-mobile.png`、`2026-06-20-theater-client-build-desktop.png`、`2026-06-20-theater-client-build-mobile.png`。本卡不改 schema、不呼叫 provider、不宣稱 production multi-character Theater ready。
