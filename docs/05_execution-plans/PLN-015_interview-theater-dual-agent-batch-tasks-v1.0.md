...
ITA-005l compliance-review queue note（2026-06-22）：已完成 disabled/no-provider 審閱候選 queue 的 source/API/UI 子切片，仍不代表正式法遵處置、real notification、legal review routing、live detection 或 external registry ready。新增 `src/domains/theater/route-b-compliance-review-queue.ts`，由已驗證 `RouteBComplianceReviewIntake` 產生跨 session `RouteBComplianceReviewQueue`，只列 session metadata、candidate counts、needs-evidence / escalation counts、top severity 與 safe candidates，並保留 no-formal/no-notification/no-provider/no-CRM-fact/no-persistence proof。新增 `GET /api/theater/route-b/compliance-review-queue` 與 `listRouteBComplianceReviewQueueForMember()`，以 current member / organization / owner / `routeBEnabled=true` scope 讀取 persisted `sceneState.feedbackReview` 後重建 intake，不信任 browser raw org/client/session/person id，且不新增 queue/candidate persistence。`/theater` 工作台右側欄新增「審閱佇列」panel，顯示待審閱、需要佐證、升級候選 counts 與進 session 檢視入口；新增 `pnpm theater:route-b-compliance-review-queue-qa` 覆蓋 domain fixture、API/repository/UI/manifest static contract、no provider/no fake `AiUsageLog`、no notification/no formal finding/no confirmed CRM fact、no raw private/provider/contact/policy/payment sentinel。

Whole-product review note（2026-06-22 after relationship confirmation cards）：第五輪校準確認前一輪 source-backed slice 已把 relationship graph -> preparation package 的顧問確認卡補上，但這些 cards 目前仍停在準備包頁面的 local advisor state；顧問從準備包建立 Route B 劇場時，stage handoff 尚未把 relationship fact / inference / unknown confirmation deck 放進 `knownMaterials`、`sourceSummary` 或旁白補問邊界。下一輪 normal loop 建議 `ITA-RCG-001 relationship confirmation deck -> Route B theater handoff grounding`：由 `buildVisitRelationshipConfirmationDeck()` 的安全輸出生成 theater handoff materials，保留 card id、person label、state、question、evidence / rationale、factStatus 與 unknown/inference 標記；不得保存 raw private transcript、不得把 inference 升格為 confirmed CRM fact、不得要求 advisor 輸入 raw session/person id、不得呼叫 provider或寫假 `AiUsageLog`。Proof 至少跑 `pnpm visit:relationship-confirmation-dry-run`、`pnpm visit:theater-handoff-dry-run`、`pnpm exec tsc --noEmit --pretty false`、`pnpm lint:changed`；若只剩 browser screenshot，可交由 operator 自行重跑，不要讓下一輪變成純證據搜集。

ITA-RCG-001 implementation note（2026-06-22）：已完成「準備包 relationship confirmation deck -> theater handoff grounding」source-backed slice。`buildVisitTheaterHandoff()` 會在 domain 端重建 `buildVisitRelationshipConfirmationDeck()` safe output，將 cards 轉成 theater `knownMaterials`，並在 `sourceSummary.evidenceSummary.relationshipConfirmation` 保留 card count、high priority、fact/inference/unknown counts、actions 與 guardrails。Unknown confirmation cards 會進旁白補問，不會升格為 confirmed CRM fact；summary 明確標示 `localAdvisorStatePersisted=false`、`providerCallAttempted=false`、`aiUsageLogWritten=false`、`writesConfirmedCrmFact=false`。此切片仍不做 card selection persistence、relationship edge schema migration、formal CRM fact writeback、provider角色回覆或 external registry publication。若要持久化顧問的 card state，需另開後續 source/API/UI slice，且 persistence allowlist 只能保存 card id / state / updatedAt / evidence refs / safe note summary。

ITA-RCG-002 card-state boundary note（2026-06-22）：已完成「relationship confirmation card state -> server-owned validation boundary」source/API/proof 子切片。新增 `src/domains/visit/relationship-confirmation-state.ts` 與 `GET/POST /api/visits/[id]/relationship-confirmation-state`，由 current member 的 owner-scoped VisitPlan 重新推導 relationship confirmation deck，再把顧問 local selection 正規化為 least-disclosure envelope；record allowlist 只保留 `cardId`、`state`、`updatedAt`、`sourceReferenceIds`、`safeNoteSummary`，safe note 會 redacted email/phone，不保存 person label、relationship detail、confirmation prompt、raw private transcript、raw provider payload 或 confirmed CRM fact。此 route 目前不寫 DB，回傳 `currentPersistence=local-only-ui-state`、`requiresProductDecision=true`、`persistedToDatabase=false`；真正 refresh/new-context persistence 仍需要產品/資料模型決策（例如 VisitPlan-owned JSON subdocument 或 dedicated table + migration/rollback proof）。

ITA-RCG-003 UI boundary bridge note（2026-06-22）：已完成「preparation page local relationship card state -> transient boundary before theater build」source/UI/proof 子切片。`/pre-visit/[planId]` 將 confirmation card state 提升到頁面層，右上「建立劇場舞台」與右側劇場卡共用同一個 `handlePrimaryAction()`；正式非 Quickstart flow 會先 POST `/api/visits/[id]/relationship-confirmation-state`，通過後才 `router.push(theaterHref)`。Relationship confirmation panel 顯示 validated record count、confirmed / ask-in-interview counts、`currentPersistence=local-only-ui-state`、`requiresProductDecision=true`、`persistedToDatabase=false`，並保留 no provider / no fake `AiUsageLog` / no DB persistence / no confirmed CRM fact write proof。新增 `pnpm visit:relationship-confirmation-state-ui-qa`，覆蓋 UI route wiring、validation-before-theater-push、panel guardrail strings、route no Prisma persistence 與 AgentFacts manifest proof command；尚未完成 refresh/new-context persistence，仍需產品/資料模型決策。
...
LV3-THEATER-MEETING-SIGNAL-RUNTIME-CONTEXT-001 acceptance note（2026-06-24）：已正式關閉 ACC-006 §5.10。Route B next-turn draft、provider prompt context、next-turn provider input 與 session UI preview 會消費 `RouteBSessionSnapshot.scene.sourceGrounding.meetingRelationshipSignals` 產出的 safe `meetingRelationshipSignalGrounding`；只回 card/unknown/narrator-question counts、safe card summaries、source/action/priority labels 與 no-write/no-provider boundary，不回 raw meeting session id、person id、sourceReferenceIds、raw transcript 或 raw provider payload。Provider disabled path 仍不呼叫 provider、不寫 fake `AiUsageLog`；live provider path 維持 success/error THEATER `AiUsageLog` gate；AgentFacts manifest 保持 `internal-only`。

Whole-product review note（2026-06-24 after runtime bridge calibration）：第五輪校準確認 REL-004a/b/c/d/e/f/g 與 meeting-signal runtime grounding 已關閉上一輪的 Route B source/runtime gaps；正式 edge table 與 relationship confirmation persistence 仍需 operator/product 決策。若下一輪仍未收到 schema approval 或 A/B/C answer，優先做 `ITA-003o Route B state proposal -> downstream advisor context bridge`：從 owner-scoped `RouteBSessionSnapshot.scene.statePatches` / persisted turn `statePatches` 產出拜訪準備或 AI meeting notes 的 advisor-context follow-up cards，保留 fact/inference/unknown、`requiresConfirmation=true`、`writesConfirmedCrmFact=false`，不寫 relationship graph、VisitPlan 或 confirmed CRM fact，不呼叫 provider、不偽造 `AiUsageLog`。剩餘純瀏覽器截圖或成本型 live click proof 可交由 operator 自行重跑，不要讓下一輪變成純證據搜集。

ITA-003o state proposal downstream advisor context note（2026-06-24）：已完成 Route B persisted state proposal -> visit preparation advisor-context bridge。新增 `src/domains/visit/route-b-state-proposal-context.ts`，將 persisted `TheaterRouteBStatePatch` 轉成 advisor follow-up / evidence-needed / next-question cards，保留 inference/unknown 標籤、`requiresConfirmation=true`、`writesRelationshipGraph=false`、`writesVisitPlan=false`、`writesConfirmedCrmFact=false`。新增 `src/lib/visits/route-b-state-proposal-context-repository.ts` 與 `GET /api/visits/[id]/route-b-state-proposal-context`，從 owner-scoped VisitPlan 重新推導 `routeBSourcePacketId`，再讀同源 Route B session 的 `sceneState.statePatches` 與 turn `statePatches`；不接受 browser raw org/client/session/person id。`/pre-visit/[planId]` 新增「劇場狀態回帶」panel，清楚標示來源為 theater state proposal 而非 confirmed relationship fact，不顯示 raw theater session/person id，不寫 relationship graph、VisitPlan 或 confirmed CRM fact。新增 `pnpm visit:route-b-state-proposal-context-qa`，並更新 `asai.visit.preparation_package` AgentFacts manifest/registry refs；仍不代表 formal `RelationshipEdge` table、relationship confirmation persistence、live provider proof 或 external registry publication 已完成。

ITA/AMM-003p AI meeting notes state proposal consumer note（2026-06-24）：已完成 Route B persisted state proposal -> AI meeting notes advisor-context bridge。正式落地面維持已接受的 `/pre-visit/[planId]/notes` + owner-scoped `MeetingWorkspace`，不採用 untracked `/notes` local prototype。Notes page 從 `GET /api/visits/[id]/route-b-state-proposal-context` 載入 Route B state proposal context，僅傳 `status`、`summary`、`routeBStateProposalContext` items 與 proof 到 MeetingWorkspace，不傳 `sourcePacketId`、theater session id 或 person id。MeetingWorkspace 新增 `meeting-route-b-state-proposal-context` 面板，將 requiresConfirmation=true 的 follow-up / evidence-needed / next-question reminders 併入 manual-note draft，並顯示 no-provider、no relationship graph write、no VisitPlan write、no direct CRM fact write guardrails。新增 `pnpm meeting:route-b-state-proposal-context-qa`，覆蓋 notes bridge、workspace panel/draft merge、BFF proof、manifest/registry refs 與 no raw id boundary；`asai.meeting.prototype` manifest 已新增 internal-only capability/action/DTO/evidence refs。仍不代表 formal `RelationshipEdge` table、relationship confirmation persistence、live provider proof 或 external registry publication 已完成。

ITA/AMM-003q meeting writeback preview bridge note（2026-06-24）：已完成 Route B persisted state proposal -> AI Meeting writeback preview context bridge。新增 `MeetingRouteBStateProposalWritebackBridge`，將已驗收的 `VisitRouteBStateProposalContext` 轉成 `MEETING_WRITEBACK_PREVIEW_CONTEXT` cards；MeetingWorkspace 在摘要與正式寫回 confirmation cards 之間顯示 `meeting-route-b-state-proposal-writeback-bridge` panel，若尚未有 persisted summary 則顯示 `SUMMARY_REQUIRED`，若摘要已存在則進入 `READY_FOR_ADVISOR_REVIEW`。此 bridge 只提供寫回預覽脈絡，仍需 advisor confirmation，且固定 no-provider、no fake `AiUsageLog`、no relationship graph write、no VisitPlan write、no confirmed CRM fact write。新增 `pnpm meeting:route-b-state-proposal-writeback-bridge-qa`，並同步 `asai.meeting.prototype` AgentFacts manifest / registry QA。仍不代表 formal `RelationshipEdge` schema、relationship confirmation persistence、live provider proof 或 external registry publication 已完成。

ITA/AMM-003r cross-flow proof note（2026-06-24）：已新增 `pnpm lv3:route-b-state-proposal-cross-flow-qa`，將 ITA-003o/p/q 收斂成一個 source-backed proof：theater persisted state proposal -> visit preparation context -> AI Meeting notes consumer -> meeting writeback preview context。此 command 會重跑 `pnpm visit:route-b-state-proposal-context-qa`、`pnpm meeting:route-b-state-proposal-context-qa`、`pnpm meeting:route-b-state-proposal-writeback-bridge-qa`，並做 manifest / registry / package source audit；總 proof 明確 no-provider、no-DB、no-browser、no relationship graph write、no VisitPlan write、no confirmed CRM fact write、no external registry publication。同步更新 `asai.theater.route_b`、`asai.visit.preparation_package`、`asai.meeting.prototype` manifest refs 與 `scripts/ai-protocol-registry-qa.ts`。仍不代表 formal `RelationshipEdge` schema、relationship confirmation persistence、live provider proof、external registry publication 或任何 CRM confirmed fact 寫入完成。
