...
- Pendo 對 onboarding 的建議是避免把導覽做成功能清單，而是引導使用者快速得到價值。落地到本案：每頁只保留「這一步看什麼」與「下一步會做什麼」，讓 demo 使用者不需要思考導覽結構。Source: https://www.pendo.io/pendo-blog/6-principles-for-effective-user-onboarding/
- Userpilot 的 flow best practices 強調清楚、精簡、可行動的下一步，並可跨 URL 做 walkthrough。落地到本案：每一頁都有固定下一步，必要時透過 URL query 自動建立 demo session。Source: https://docs.userpilot.com/in-app-engagement/flows/getting-started/best-practices and https://docs.userpilot.com/article/18-interactive-walkthroughs-that-spans-across-different-urls
...
6. Quickstart 的 primary CTA 一律使用「下一步：...」語氣；手機底部固定下一步，桌面在導覽卡右上角提供同一動作。
...
## Next-Only Interaction Model

Quickstart 不再要求使用者理解左側導覽或頁面功能名稱；每一步都提供固定的「下一步」：

- `/pilot` 下一步到 `/pre-visit?demo=quickstart`。
- `/pre-visit?demo=quickstart` 的 modal CTA 會建立王大明加保規劃，並進入 `/pre-visit/[planId]?demo=quickstart`。
- `/pre-visit/[planId]` 下一步會自動建立 SPIN session，進入 `/spin/[sessionId]?demo=quickstart`。
- `/spin/[sessionId]` 下一步會自動建立 Theater session，進入 `/theater/[sessionId]?demo=quickstart`。
- `/theater/[sessionId]` 下一步會自動生成 Report，進入 `/reports/[reportId]?demo=quickstart`。
- `/reports/[reportId]` 下一步回到 `/dashboard?demo=quickstart`，完成體驗閉環。
