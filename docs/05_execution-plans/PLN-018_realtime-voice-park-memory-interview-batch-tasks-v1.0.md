...
| PIM-011 | Post-visit quick-capture -> Park memory bridge | [x] | PIM-006、PIM-010、BFF-104；BFF/API + UI selector proof done |
...
完成註記（2026-06-21 PIM-011c）：新增 `/pre-visit/[planId]/notes` quick-capture selector 與 `pnpm interview:quick-capture-ui-qa`。UI 以 current workspace context 自動帶入客戶/拜訪，顧問只選「保持私人草稿 / 歸客戶 / 歸拜訪 / 轉待確認」與 reason/riskAccepted；response panel 只顯示 safe DTO 摘要、memory/state proposal ids、`scope=server_session`、`provider=none`，不 echo raw note/private sentinel。Proof 覆蓋 desktop high-sensitive visit capture 缺 approval 409、補 approval 後 READY 201、mobile follow-up review state proposal、desktop/mobile no overflow、result panel/API response no raw note echo、`AiUsageLog` 147->147；BFF regression 與 AI usage route audit 通過。截圖：`docs/06_audits-and-reports/screenshots/pim/pim-011c-quick-capture-ui/pim-011c-notes-desktop.png`、`docs/06_audits-and-reports/screenshots/pim/pim-011c-quick-capture-ui/pim-011c-notes-mobile.png`。

完成註記（2026-06-23 PIM/AMM quick-note writeback bridge）：新增 `VisitMeetingQuickNoteWritebackBridgeDto` 與 `buildVisitMeetingQuickNoteWritebackBridge()`，讓 `/api/visits/[id]/meeting-quick-notes` 回傳「已同步筆記 -> 需先生成 persisted meeting summary -> 再進 writeback confirmation cards」的安全橋接資訊。`/pre-visit/[planId]/notes` 的 quick-note result panel 顯示 `post-visit-meeting-writeback-bridge`，並引導到 accepted `CLIENT_MEETING` workspace；橋接只揭露 workspace href、summary/writeback endpoint pattern、summary required、advisor confirmation、sensitive reason/riskAccepted requirement 與 no direct CRM fact write guardrails，不接受 browser-supplied session id、不呼叫 provider、不儲存 raw provider/private transcript payload、不寫 confirmed CRM fact。新增 `pnpm meeting:quick-note-writeback-bridge-qa`，並同步 `asai.meeting.prototype` AgentFacts manifest / registry QA；剩餘 live browser/API/DB evidence 可由 operator 視需要重跑 `pnpm meeting:notes-compat-qa` 或 `pnpm meeting:workspace-writeback-ui-qa`。
