...
- BFF-001 盤點後的 production-facing source blockers：`/reports` library/detail/share action 仍是 local report store、`/issues` 仍是 `MOCK_ISSUES`、`/team` 仍是 `MOCK_TEAM_MEMBERS`、admin/pilot 頁仍直接讀 demo seed、SPIN session 仍有 `/api/mock/ai/spin-outline` fallback、notification reminder route 仍是 mock success。這些不得被用作正式 LV3 server-owned proof。
