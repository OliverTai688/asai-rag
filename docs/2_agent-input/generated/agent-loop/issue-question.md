...
- BFF-105 後的 production-facing source blockers：`/issues` 仍是 `MOCK_ISSUES`、`/team` 仍是 `MOCK_TEAM_MEMBERS`、dashboard 仍有 client-side aggregation、admin/pilot 頁仍直接讀 demo seed、SPIN session 仍有 `/api/mock/ai/spin-outline` fallback、notification reminder route 仍是 mock success。這些不得被用作正式 LV3 server-owned proof。Reports library/detail/share action 已可作 member-scoped BFF proof；`/api/ai/report` provider generation 仍需 BFF-202 持續守住 AiUsageLog/usage audit。
