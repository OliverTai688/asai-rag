...
`BFF-402f` server query boundary proof is acceptable before real ECPay query confirmation only if:

- `/api/billing/ecpay/query` keeps server-owned auth/scope and does not trust client-provided organization, amount, provider status, return URL, or browser-side query result.
- The query DTO includes a versioned server query boundary contract with `providerAttempted=false`, `confirmationReceived=false`, `paymentTransactionUpsertAttempted=false`, `organizationPlanUpdated=false`, and fake `AiUsageLog` disallowed.
- The boundary references the ledger idempotency contract and states that confirmed transaction/query activation still requires real provider query response validation plus `PaymentTransaction` persistence/upsert.
- The proof must keep real provider query confirmation blocked; guarded-disabled query boundary must not be counted as production payment readiness.
- The slice must not create subscription orders, payment transactions, ledger rows, provider calls, fake `AiUsageLog`, real payment, refund, void, or real plan activation.

Evidence（2026-06-23 BFF-402f）：`pnpm billing:ecpay-query-boundary-qa` passed static source checks; `BILLING_ECPAY_DISABLED_QA_BASE_URL=http://127.0.0.1:3063 pnpm billing:ecpay-disabled-qa` passed guarded route proof; `pnpm bff:release-readiness-qa` passed with the new `ecpay_server_query_guarded_boundary` subgate while `ecpay_server_query_confirmation` remains blocked/provider_env. Proof covers server-owned endpoint, browser query disabled, client-supplied organization/amount untrusted, no provider call, no provider confirmation, no `PaymentTransaction` upsert, no organization plan mutation, no fake `AiUsageLog`, and no raw payment/provider/private sentinel. This does not satisfy real ECPay server-to-server query confirmation or production payment enablement.

`BFF-402g` PaymentTransaction persistence guarded contract proof is acceptable before real DB upsert only if:

- A versioned `asai.billing.payment_transaction_persistence.v1` contract defines `PaymentTransaction` unique scope as `organizationId + provider + merchantTradeNo`, `upsert_when_enabled`, accepted verified statuses `PAID` / `QUERY_CONFIRMED`, and allowlisted provider summary only.
- ECPay notify/query disabled DTOs include `transactionPersistence` and keep `paymentTransactionUpsertAttempted=false`, `subscriptionOrderUpdated=false`, `organizationPlanUpdated=false`, `redirectOnlyActivationAllowed=false`, no provider call, no DB write, and fake `AiUsageLog` disallowed.
- Notify persistence requires CheckMacValue validation; query persistence requires server query confirmation; neither path trusts client-supplied organization, amount, provider status, browser redirect, or return URL as activation proof.
- The contract references the ledger version/scope but does not claim live `PaymentTransaction` DB write proof. Release readiness may mark `payment_transaction_persistence_guarded_contract` as pass, but `payment_transaction_persistence` must remain blocked/db until a live DB upsert/write proof exists.
- Response, source, and proof output must not expose HashKey, HashIV, CheckMacValue, provider raw payload, provider account identifiers, payment token, card data, raw payment data, raw cookie, secret, token, OTP, raw private transcript, or env values.

Evidence（2026-06-23 BFF-402g）：`pnpm billing:payment-transaction-persistence-qa` passed static source checks; `BILLING_ECPAY_DISABLED_QA_BASE_URL=http://127.0.0.1:3064 pnpm billing:ecpay-disabled-qa` passed guarded runtime/source proof; `pnpm billing:ledger-idempotency-qa` and `pnpm bff:release-readiness-qa` passed with `payment_transaction_persistence_guarded_contract` as pass while live `payment_transaction_persistence` remains blocked/db. Proof covers unique scope, verified write preconditions, no DB upsert, no order update, no organization plan mutation, no raw provider payload/checksum/private data storage, no provider call, no fake `AiUsageLog`, and payment/private sentinel 0. This does not satisfy real PaymentTransaction DB persistence/upsert, real ECPay query confirmation, confirmed activation, refund/void/manual review, or production payment enablement.

`BFF-402i` PaymentTransaction upsert payload boundary proof is acceptable before real DB upsert only if:

- A versioned `asai.billing.payment_transaction_upsert_boundary.v1` boundary defines `PaymentTransaction` allowlisted create/update columns and keeps server-resolved `organizationId + provider + merchantTradeNo` as the unique scope.
- The draft builder may output notify/query payload shape for review, but it must keep `enabled=false`, `dbWriteAttempted=false`, `paymentTransactionUpsertAttempted=false`, `providerCallAttempted=false`, and `organizationPlanUpdated=false`.
- Notify drafts require checksum validation; query drafts require server query confirmation; neither path trusts client-supplied organization, amount, provider status, browser redirect, or return URL.
- Release readiness may mark `payment_transaction_upsert_payload_boundary` as pass, but `payment_transaction_persistence` must remain blocked/db until a live DB upsert/write proof exists.
- Source/proof output must not expose HashKey, HashIV, CheckMacValue, provider raw payload, provider account identifiers, payment token, card data, raw payment data, raw cookie, secret, token, OTP, raw private transcript, or env values.

Evidence（2026-06-23 BFF-402i）：`pnpm billing:payment-transaction-upsert-boundary-qa` passed static source checks plus TypeScript dry-run for notify/query draft payloads; `pnpm bff:release-readiness-qa` passed with `payment_transaction_upsert_payload_boundary` as pass while live `payment_transaction_persistence` remains blocked/db. Proof covers allowlisted create/update columns, server-resolved organization scope, accepted verified statuses, no `.upsert()`, no DB write, no provider call, no raw payment/private data, no fake `AiUsageLog`, and no organization plan mutation. This does not satisfy real PaymentTransaction DB persistence/upsert, real ECPay query confirmation, confirmed activation, refund/void/manual review, or production payment enablement.

`BFF-402h` confirmed activation guarded contract proof is acceptable before real organization plan mutation only if:

- A versioned `asai.billing.confirmed_activation.v1` contract defines `Organization.plan` activation target, `update_when_enabled`, accepted verified statuses `PAID` / `QUERY_CONFIRMED`, and prerequisites for confirmed ledger plus PaymentTransaction persistence/write proof.
- ECPay notify/query disabled DTOs include `confirmedActivation` and keep `planActivated=false`, `organizationPlanUpdated=false`, `paymentTransactionUpsertAttempted=false`, `subscriptionOrderUpdated=false`, `redirectOnlyActivationAllowed=false`, `browserPlanAssumptionsAllowed=false`, no provider call, no DB write, and fake `AiUsageLog` disallowed.
- Notify activation requires checksum validation; query activation requires server query confirmation; neither path trusts browser payment result, return URL, client-supplied organization/amount/plan, localStorage plan, or redirect-only state as activation proof.
- Release readiness may mark `confirmed_activation_guarded_contract` as pass, but `confirmed_activation` must remain blocked/source until live confirmed transaction/query, real PaymentTransaction write, and organization plan mutation proof exist.
- Response, source, and proof output must not expose HashKey, HashIV, CheckMacValue, provider raw payload, provider account identifiers, payment token, card data, raw payment data, raw cookie, secret, token, OTP, raw private transcript, or env values.

Evidence（2026-06-23 BFF-402h）：`pnpm billing:confirmed-activation-qa` passed static source checks; `BILLING_ECPAY_DISABLED_QA_BASE_URL=http://127.0.0.1:3065 pnpm billing:ecpay-disabled-qa` passed guarded runtime/source proof; `pnpm billing:payment-transaction-persistence-qa`、`pnpm billing:ledger-idempotency-qa` and `pnpm bff:release-readiness-qa` passed with `confirmed_activation_guarded_contract` as pass while live `confirmed_activation` remains blocked/source. Proof covers activation prerequisites, blocked client/redirect/localStorage signals, no organization plan mutation, no raw provider/payment/checksum/private data storage, no provider call, no fake `AiUsageLog`, and payment/private sentinel 0. This does not satisfy real PaymentTransaction DB persistence/upsert, real ECPay provider query confirmation, live organization plan mutation, refund/void/manual review, or production payment enablement.
