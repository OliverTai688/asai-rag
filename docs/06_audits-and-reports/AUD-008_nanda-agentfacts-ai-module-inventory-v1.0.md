...
| --- | --- | --- | --- | --- | --- |
| `asai.chat.assistant` | `src/app/api/ai/chat/route.ts`; `src/lib/assistant/assistant-chat-repository.ts`; `src/lib/assistant/assistant-tools.ts` | `ai:bff-audit` sees session/quota/success-error usage evidence; NAP-003a manifest now points to tool allowlist and conversation persistence refs. | Completed in NAP-003a: `proof.sourceAdoption.status=adopted`. | Next only if assistant tool contracts change. | `pnpm ai:bff-audit`; `pnpm ai:protocol-registry-qa` |
| `asai.visit.preparation_package` | `src/app/api/ai/visit/route.ts`; `src/domains/visit/ai-evidence-dto.ts`; `src/lib/visits/visit-plan-repository.ts` | `bff:visit-report-ai-qa` proves success/error usage and provider-safe snapshot; NAP-003a manifest now points to snapshot, evidence DTO, reasoning, and VisitPlan persistence refs. | Completed in NAP-003a: `proof.sourceAdoption.status=adopted`. | Next only if visit-to-theater handoff adds new writeback surface. | `pnpm ai:protocol-registry-qa`; provider-enabled proof remains `pnpm bff:visit-report-ai-qa` |
| `asai.report.generation` | `src/app/api/ai/report/route.ts`; `src/lib/report/report-repository.ts`; `src/lib/report/report-dto.ts` | `bff:visit-report-ai-qa` proves success/error usage and JSON DTO; `bff:reports-qa` proves deterministic reports/share DTO separation; NAP-003a manifest now names both source boundaries. | Completed in NAP-003a: `proof.sourceAdoption.status=adopted`. | Next only if public share/export registry metadata is introduced. | `pnpm ai:protocol-registry-qa`; `pnpm bff:reports-qa`; provider-enabled proof remains `pnpm bff:visit-report-ai-qa` |
| `asai.spin.advisor` | `src/app/api/ai/spin/route.ts`; `src/app/api/ai/spin-suggestions/route.ts`; `src/lib/spin/spin-session-repository.ts` | `spin:source-truth-qa` and `ai:bff-audit` prove BFF source and provider usage posture; NAP-003a manifest now names the state-machine refs. | Completed in NAP-003a: `proof.sourceAdoption.status=adopted`. | Next only if SPIN persistence/state machine changes. | `pnpm spin:source-truth-qa`; `pnpm ai:protocol-registry-qa` |
| `asai.interview.companion` | `src/app/api/ai/interview/route.ts`; `src/app/api/ai/interview/outputs/route.ts`; `src/lib/interview/interview-ai-repository.ts`; `src/lib/interview/interview-persistence-repository.ts`; `src/lib/interview/interview-writeback-repository.ts`; `src/domains/interview/writeback-boundary.ts` | PIM reports prove Park memory, reflection/planning, writeback, and draft writeback; NAP-003b manifest now points to provider turns, deterministic session/memory/reflection routes, writeback confirmation, and draft handoff. | Completed in NAP-003b: `proof.sourceAdoption.status=adopted`. | Next only if interview writeback target contract changes. | `pnpm ai:protocol-registry-qa`; `pnpm interview:cross-mode-qa`; `pnpm interview:draft-writeback-qa` |
| `asai.interview.quick_capture` | `src/app/api/ai/interview/quick-captures/route.ts`; `src/domains/interview/quick-capture.ts`; `src/lib/interview/interview-persistence-repository.ts`; `src/app/(dashboard)/pre-visit/[planId]/notes/page.tsx` | `interview:quick-capture-bff-qa` / `interview:quick-capture-ui-qa` prove server scope override, high-sensitive gate, no raw note echo, no provider, and unchanged usage; NAP-003b manifest now names these as accepted source evidence. | Completed in NAP-003b: `proof.sourceAdoption.status=adopted`. | Next only if notes/meeting surfaces add a new capture origin. | `pnpm ai:protocol-registry-qa`; `pnpm interview:quick-capture-bff-qa`; `pnpm interview:quick-capture-ui-qa` |
| `asai.interview.realtime_voice` | `src/app/api/ai/interview/realtime-session/route.ts`; `src/app/api/ai/interview/realtime-events/route.ts`; `src/app/api/ai/interview/transcribe/route.ts`; `src/app/api/ai/interview/transcribe-realtime-session/route.ts`; `src/lib/interview/realtime-bff.ts` | Realtime BFF QA proves dry-run token/session, event mirror payload bounds, and no server secret leakage; NAP-003b manifest now separates dry-run/session-marker, event mirror, transcription provider, and live WebRTC blocker. | Completed in NAP-003b: `proof.sourceAdoption.status=adopted`; live WebRTC proof still blocked. | Next code proof only after live WebRTC/browser session acceptance or provider approval. | `pnpm ai:protocol-registry-qa`; `pnpm interview:realtime-bff-qa` |
| `asai.theater.legacy` | `src/app/api/ai/theater/route.ts`; `src/app/api/ai/theater/score/route.ts`; `src/app/api/ai/theater-build/route.ts`; `src/lib/theater/theater-ai-repository.ts`; `src/lib/theater/theater-build-ai-repository.ts` | `ai:bff-audit` sees provider-ready guarded legacy routes; NAP-003c manifest now names guarded route owners, Route B required gate, and success/error usage repositories. | Completed in NAP-003c: `proof.sourceAdoption.status=adopted`; legacy remains guarded and is not Route B production runtime. | Next only if legacy demo gate or protected scoring contract changes. | `pnpm ai:bff-audit`; `pnpm ai:protocol-registry-qa` |
| `asai.theater.route_b` | `src/app/api/theater/route-b/runtime/route.ts`; `src/app/api/theater/route-b/sessions/**`; `src/domains/theater/route-b-handoff.ts`; `src/domains/theater/route-b-session.ts`; `src/lib/theater/route-b-boundary.ts`; `src/lib/theater/route-b-session-bff-repository.ts`; `src/lib/theater/route-b-session-repository.ts` | `ITA-003g` aligned runtime preflight; session/interactions/stage map have no-provider proof; NAP-003c manifest now names runtime preview, handoff boundary, session/turn owner scope, private/group visibility, and state proposal boundary. | Completed in NAP-003c: `proof.sourceAdoption.status=adopted`; live provider success/error `AiUsageLog`, AI role orchestration, and five-view feedback runtime remain open blockers. | Next code slice should be NAP-005 local adapter proof or provider-disabled role orchestration contract; provider-approved success/error proof needs operator approval. | `pnpm theater:route-b-runtime-qa`; `pnpm theater:route-b-interaction-qa`; `pnpm ai:protocol-registry-qa` |
| `asai.rag.private_beta` | `src/app/api/rag/route.ts`; `src/domains/rag/services/rag.service.ts`; `scripts/rag-launch-posture-qa.mjs` | `rag:launch-posture-qa` proves guarded-disabled posture; NAP-003c manifest now names no-provider route, launch posture, usage counter, and ingestion blocker. | Completed in NAP-003c: `proof.sourceAdoption.status=adopted`; RAG remains disabled and does not claim client/high-sensitive ingestion. | Next only after accepted ingestion/privacy proof or local-only adapter least-disclosure export. | `pnpm rag:launch-posture-qa`; `pnpm ai:protocol-registry-qa` |
...
### NAP-003a Completion Evidence

Implementation/proof update on 2026-06-21:

- `asai.chat.assistant`, `asai.visit.preparation_package`, `asai.report.generation`, and `asai.spin.advisor` now declare `proof.sourceAdoption.status=adopted` in `src/domains/ai-protocol/manifest.ts`.
- Source adoption evidence includes owner refs, evidence refs, and notes for route ownership, DTO/evidence boundaries, quota posture, success/error `AiUsageLog` policy, and SPIN state-machine preservation.
- `scripts/ai-protocol-registry-qa.ts` now enforces the NAP-003a source-adoption requirements for these four agent ids.
- The slice remains internal-only: no external registry publication, no signing, no public discovery endpoint, no cross-org access, and no provider calls.
- `pnpm bff:visit-report-ai-qa` is kept as accepted provider-enabled proof for VISIT/REPORT, but this no-provider slice intentionally does not run it because it triggers provider success paths and writes VISIT/REPORT usage rows.

### NAP-003b Completion Evidence

Implementation/proof update on 2026-06-21:

- `asai.interview.companion`, `asai.interview.quick_capture`, and `asai.interview.realtime_voice` now declare `proof.sourceAdoption.status=adopted` in `src/domains/ai-protocol/manifest.ts`.
- Interview companion evidence names provider turn/output routes, success/error `AiUsageLog` helpers, deterministic session/memory/reflection persistence, writeback confirmation, and visit/theater draft handoff boundaries.
- Quick-capture evidence names the deterministic no-provider BFF/UI source, server-derived scope, high-sensitive approval gate, no raw note echo posture, and "do not write confirmed CRM fact" boundary.
- Realtime voice evidence names dry-run ephemeral token/session marker, event mirror allowlist, credential/audio payload rejection, transcription provider separation, and the live WebRTC proof blocker.
- `scripts/ai-protocol-registry-qa.ts` now enforces source-adoption requirements for NAP-003a and NAP-003b agent ids.
...
### NAP-003c Completion Evidence

Implementation/proof update on 2026-06-21:

- `asai.theater.legacy`, `asai.theater.route_b`, and `asai.rag.private_beta` now declare `proof.sourceAdoption.status=adopted` in `src/domains/ai-protocol/manifest.ts`.
- Legacy Theater evidence preserves guarded provider-ready route ownership, protected legacy persona/scoring contracts, Route B required gate posture, and success/error usage repository ownership.
- Route B evidence names runtime preflight source alignment, handoff validation, deterministic session/turn persistence, owner-scoped private/group visibility, state proposal boundaries, and the provider enablement blocker for director/character/feedback runtime.
