...
- ✅ Production-minimum pass：`/api/ai/chat`、`/api/ai/interview`、`/api/ai/interview/outputs`、`/api/ai/theater`、`/api/ai/theater/score`。
- ⛔ 必須改造或下線後才可 public：`/api/ai/spin`、`/api/ai/spin-suggestions`、`/api/ai/visit`、`/api/ai/report`。
...
> 註：DB aggregate 只列 module/provider/count/time window，不輸出 raw error、request id 或 private payload。CHAT/THEATER 的既有 proof 記錄在 `RPT-003` 與 `PLN-017`，本次 DB current-month target 只保留目前 target 的 `INTERVIEW` aggregate。
...
| `/api/ai/visit` | `VISIT` | `OPENAI` | ✅ | ❌ | ❌ | ❌ | ❌ | Gap |
| `/api/ai/report` | `REPORT` | `OPENAI` | ✅ | ❌ | ❌ | ❌ | ❌ | Gap |
...
1. Convert `/api/ai/visit` and `/api/ai/report` first, because they are user-facing generation routes and currently call OpenAI without session scope or cost logging.
2. Either retire legacy `/api/ai/spin` / `/api/ai/spin-suggestions` from public runtime or convert them to the same session/quota/usage contract used by `CHAT` and `INTERVIEW`.
3. Decide `/api/rag` launch posture: keep disabled/placeholder behind guard, or implement real provider/vector path with `AiUsageLog`.
...
- Overall: `gaps_found`.
- Routes with gaps: `/api/ai/spin`, `/api/ai/spin-suggestions`, `/api/ai/visit`, `/api/ai/report`, `/api/rag`.
