...
console.log(
  JSON.stringify(
    {
      status: "pass",
      proof: "meeting-route-b-state-proposal-writeback-bridge-qa",
...
function runSourceContractAudit() {
  const bridge = readSource("src/domains/interview/meeting-route-b-state-proposal-writeback-bridge.ts");
  ...
  assertIncludes(
    ...
    "domain helper builds the meeting writeback preview bridge",
  );
  assertIncludes(
    bridge,
    "READY_FOR_ADVISOR_REVIEW",
    "bridge has an explicit ready-for-review status",
  );
  assertIncludes(
    bridge,
    "SUMMARY_REQUIRED",
    "bridge blocks writeback preview until a persisted meeting summary exists",
  );
  assertIncludes(
    bridge,
    "MEETING_WRITEBACK_PREVIEW_CONTEXT",
    "bridge targets meeting writeback preview context only",
  );
  ...
  assertIncludes(
    meetingWorkspace,
    "buildMeetingRouteBStateProposalWritebackBridge",
    "workspace consumes the bridge helper",
  );
  assertIncludes(
    meetingWorkspace,
    "data-testid=\"meeting-route-b-state-proposal-writeback-bridge\"",
    "workspace renders writeback preview bridge panel",
  );
  assertIncludes(
    meetingWorkspace,
    "MEETING_WRITEBACK_PREVIEW_CONTEXT",
  ...
  assertIncludes(
    ...
    "AgentFacts manifest records state proposal writeback bridge capability",
  );
  assertIncludes(
    manifest,
    "route-b-state-proposal-writeback-preview-bridge",
    "AgentFacts manifest records writeback preview bridge action",
  );
  assertIncludes(
    manifest,
    "MeetingRouteBStateProposalWritebackBridge",
    "AgentFacts manifest records bridge DTO",
  );
  assertIncludes(
    manifest,
    "pnpm meeting:route-b-state-proposal-writeback-bridge-qa",
    "AgentFacts manifest advertises bridge proof command",
  );
  assertIncludes(
    registryQa,
    "assertMeetingRouteBStateProposalWritebackBridge",
    "registry QA enforces state proposal writeback bridge evidence",
  );
  assertIncludes(
    registryQa,
    "pnpm meeting:route-b-state-proposal-writeback-bridge-qa",
    "registry QA expects bridge proof command",
  );
  assertIncludes(
    packageJson,
    "\"meeting:route-b-state-proposal-writeback-bridge-qa\"",
    "package.json registers bridge proof command",
  );
}
